...
func submitExecutionResult(ctx context.Context, caller executionCallFace, cause message.Cause, controlActor actor.ActorID,
	...
	switch resultKind {
	case "listing_page", "listing_completion", "diagnostic", "source_validation", "detail", "source_discovery", "company_import_preview_chunk", "company_import_preview_completion", "company_import_apply":
	default:
	...
	present := map[string]bool{"listing_page": len(decoded.Page) != 0, "listing_completion": len(decoded.Completion) != 0,
		...
		"source_validation": len(decoded.SourceValidation) != 0,
