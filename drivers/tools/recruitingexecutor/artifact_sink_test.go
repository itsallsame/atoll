...
func TestAtollArtifactSinkCreatesStableFileResource(t *testing.T) {
	...
	if !writer.committed || !bytes.Equal(writer.Bytes(), body) || ref.ArtifactID == "" || ref.ContentHash == "" ||
		ref.ObjectRef != resources.created.String() || len(ref.ObjectRef) < len("daemon://worker-a/recruiting/recruiting-artifacts/") ||
		ref.ObjectRef[:len("daemon://worker-a/recruiting/recruiting-artifacts/")] != "daemon://worker-a/recruiting/recruiting-artifacts/" {
