...
func manifest() introspect.Manifest {
	result := introspect.Manifest{
		...
		Words: map[string]introspect.WordSpec{
			...
			TypeOnboardingStatus:               {Description: "look up onboarding progress and typed validated recruitment URLs by company name; no internal ID is required"},
			TypeOnboardingMaterialize:          {Description: "materialize every typed validated URL from a completed company-name onboarding Mission as an idempotent candidate Source; no internal ID is required"},
