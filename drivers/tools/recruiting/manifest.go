...
func manifest() introspect.Manifest {
	result := introspect.Manifest{
		...
		Words: map[string]introspect.WordSpec{
			...
			TypeOnboardingAdvance:              {Description: "advance one safe asynchronous discovery step from only a company name; browser network responses are retained directly as immutable evidence"},
	...
	inputSchemas := map[string]string{
		...
		TypeDeepDiscoveryBrowserObserve: mutationSchema(`,"probe_id":{"type":"string"},"work_id":{"type":"string"},"url":{"type":"string"},"wait_selector":{"type":"string"},"scroll_repeats":{"type":"integer","minimum":0,"maximum":10},"follow_link_selector":{"type":"string"},"browser_query":{"type":"object","additionalProperties":false,"required":["method","endpoint_path"],"properties":{"method":{"type":"string","const":"POST"},"endpoint_path":{"type":"string"}}},"listing_advance":`+listingAdvanceSchema()+`,"priority":{"type":"integer","minimum":-100,"maximum":100},"deadline_at":{"type":"string"}`,
...
func deepDiscoveryCheckpointSchema() string {
	coverage := `{"type":"object","additionalProperties":false,"required":["identity_scoped","brands_reviewed","sites_enumerated","sites_explored","pools_detected","candidates_validated","blindspots_reviewed","critical_gap_count"],"properties":{"identity_scoped":{"type":"boolean"},"brands_reviewed":{"type":"boolean"},"sites_enumerated":{"type":"boolean"},"sites_explored":{"type":"boolean"},"pools_detected":{"type":"boolean"},"candidates_validated":{"type":"boolean"},"blindspots_reviewed":{"type":"boolean"},"critical_gap_count":{"type":"integer","minimum":0},"social_coverage":{"type":"string","enum":["covered","not_found_after_search","excluded"]},"campus_coverage":{"type":"string","enum":["covered","not_found_after_search","excluded"]},"intern_coverage":{"type":"string","enum":["covered","not_found_after_search","excluded"]}}}`
	listProof := `{"type":"object","additionalProperties":false,"required":["is_company_page","is_job_listing","has_active_postings","listing_artifact_id","detail_artifact_id","sample_job_key","sample_detail_url","detail_url_pattern","identity_source","identity_path","navigation_path"],"properties":{"is_company_page":{"type":"boolean","const":true},"is_job_listing":{"type":"boolean","const":true},"has_active_postings":{"type":"boolean","const":true},"listing_artifact_id":{"type":"string"},"detail_artifact_id":{"type":"string"},"sample_job_key":{"type":"string"},"sample_detail_url":{"type":"string"},"detail_url_pattern":{"type":"string","description":"Observed absolute pattern containing exactly one {value}; never infer it from an API ID alone."},"identity_source":{"type":"string","enum":["dom_href","dom_attribute","api_field"]},"identity_path":{"type":"string","description":"Observed DOM attribute/selector or API response pointer that supplied the job identity."},"navigation_path":{"type":"array","minItems":2,"maxItems":20,"items":{"type":"string"},"description":"Observed path from official recruitment entry through the list to the clicked detail page."}}}`
	node := `{"type":"object","additionalProperties":false,"required":["ref","kind","canonical_value","label","state","sensor","basis"],"properties":{"ref":{"type":"string","description":"Checkpoint-local readable handle used by edges; the Actor generates the durable node ID."},"kind":{"type":"string","enum":["company","brand","legal_entity","domain","site","listing_pool","api_endpoint","list_url","blindspot"]},"canonical_value":{"type":"string"},"label":{"type":"string"},"state":{"type":"string","enum":["candidate","validated","rejected","excluded"]},"sensor":{"type":"string","enum":["web_search","official_site","sitemap","robots","browser","network","ats_fingerprint","detail_reverse","human"]},"evidence_url":{"type":"string"},"evidence_artifact_id":{"type":"string"},"basis":{"type":"string"},"recruitment_type":{"type":"string","enum":["social","campus","intern","special","all","unknown"],"description":"Required and non-unknown for a validated list_url; classify from page or network evidence, never from URL spelling alone."},"special_program":{"type":"string","description":"Required only when recruitment_type is special; retain the programme name shown by the site."},"list_proof":` + listProof + `}}`
...
func recipePrepareSchema() string {
	mapping := `{"type":"object","additionalProperties":false,"properties":{"collection":{"type":"string","description":"Listing only: JSON pointer to the job array in the verified response preview."},"collection_root":{"type":"boolean","const":true,"description":"Listing only: true when the verified response root itself is the job array."},"identity_pointer":{"type":"string","description":"Listing only: item-relative stable job ID pointer."},"detail_url_pointer":{"type":"string","description":"Listing only: item-relative detail URL or ID pointer."},"detail_url_template":{"type":"string","description":"Listing only: optional absolute URL containing exactly one {value}."},"title_pointer":{"type":"string"},"activity_pointer":{"type":"string"},"activity_time_format":{"type":"string","enum":["rfc3339","utc_datetime"]},"exclude_pinned_pointer":{"type":"string"},"boundary_mode":{"type":"string","enum":["frontier_keys","activity_time"]},"fields":{"type":"object","minProperties":1,"maxProperties":64,"additionalProperties":{"type":"string","minLength":1,"maxLength":500},"description":"Detail only: semantic field names mapped to selectors observed in the completed browser Probe."},"attributes":{"type":"object","maxProperties":64,"additionalProperties":{"type":"string","minLength":1,"maxLength":100},"description":"Detail only: optional safe HTML attribute per semantic field; text is used by default."},"wait_selector":{"type":"string","minLength":1,"maxLength":500,"description":"Detail only: rendered root selector that proves the page is ready."}}}`
	return `{"type":"object","additionalProperties":false,"required":["command_id","target","expected_version","reason","probe_id","mapping"],"properties":{"command_id":{"type":"string"},"target":{"type":"object","additionalProperties":false,"required":["target_type","target_id"],"properties":{"target_type":{"type":"string","const":"source"},"target_id":{"type":"string"}}},"expected_version":{"type":"integer","minimum":1},"reason":{"type":"string"},"probe_id":{"type":"string"},"recipe_kind":{"type":"string","enum":["listing","detail"],"description":"Defaults to listing for compatible callers."},"body_hash":{"type":"string","description":"Listing only: selected public-query response body hash."},"probe_content_hash":{"type":"string","description":"Detail only: terminal DOM Artifact hash from the exact sample Job Probe."},"sample_job_id":{"type":"string","description":"Detail only: pending Job proven by the completed Probe."},"mapping":` + mapping + `},"oneOf":[{"properties":{"recipe_kind":{"const":"listing"},"mapping":{"required":["identity_pointer","detail_url_pointer"],"oneOf":[{"required":["collection"],"not":{"required":["collection_root"]}},{"required":["collection_root"],"not":{"required":["collection"]}}]}},"required":["body_hash"],"not":{"anyOf":[{"required":["probe_content_hash"]},{"required":["sample_job_id"]}]}},{"properties":{"recipe_kind":{"const":"detail"},"mapping":{"required":["fields","wait_selector"]}},"required":["recipe_kind","probe_content_hash","sample_job_id"],"not":{"required":["body_hash"]}}]}`
}

func listingAdvanceSchema() string {
	endProof := `{"type":"object","additionalProperties":false,"required":["kind"],"properties":{"kind":{"type":"string","enum":["single_batch","response_false","response_empty","selector_absent_or_disabled","stable_no_progress"]},"pointer":{"type":"string"},"selector":{"type":"string"}}}`
	return `{"type":"object","additionalProperties":false,"required":["kind","progress_proof","end_proof","wait_timeout_ms","max_advances","max_no_progress"],"properties":{"kind":{"type":"string","enum":["none","click","scroll_page","scroll_container"]},"selector":{"type":"string"},"progress_proof":{"type":"array","minItems":1,"maxItems":8,"uniqueItems":true,"items":{"type":"string","enum":["response","cursor","job_identity","url","list_count"]}},"end_proof":` + endProof + `,"wait_timeout_ms":{"type":"integer","minimum":100,"maximum":30000},"max_advances":{"type":"integer","minimum":0,"maximum":1000},"max_no_progress":{"type":"integer","minimum":0,"maximum":10}}}`
}
