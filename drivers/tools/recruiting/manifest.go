...
func manifest() introspect.Manifest {
	result := introspect.Manifest{
		...
		Words: map[string]introspect.WordSpec{
			...
			TypeDeepDiscoveryPublicQueryInspect: {Description: "inspect a bounded structural preview of one verified public JSON response so an Agent can derive evidence-backed field paths without direct Resource access"},
	...
	inputSchemas := map[string]string{
		...
		TypeDeepDiscoveryPublicQueryInspect: `{"type":"object","additionalProperties":false,"required":["id"],"properties":{"id":{"type":"string"}}}`,
