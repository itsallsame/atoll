...
func insertBaselineWith(ctx context.Context, executor interface {
	...
	if baseline.Version != 1 || baseline.Status != model.BaselineListing || baseline.ListingAttemptID != "" {
...
INSERT INTO recruiting_baseline_generations(
  source_id, work_id, baseline_generation, company_version, source_version, listing_attempt_id, generation_status, listing_finalized,
  ...
  details_accounted, detail_exceptions, version, state_json, created_at, updated_at
		...
		nullableUint(baseline.SourceVersion), nullableString(baseline.ListingAttemptID), baseline.Status, baseline.ListingFinalized,
...
WHERE source_id = ? AND baseline_generation = ? FOR UPDATE`, sourceID, generation).Scan(&baselineState); err != nil {
		...
		if baseline.Status != model.BaselineListing || baseline.ListingFinalized || baseline.WorkID != "" {
			_ = tx.Rollback()
			return chunks, fmt.Errorf("baseline staging is finalized or requires an executable Attempt")
		}
...
ON DUPLICATE KEY UPDATE
  attempt_id = NULL, observation_id = VALUES(observation_id), row_json = VALUES(row_json), staged_at = VALUES(staged_at)`,
...
WHERE source_id = ? AND baseline_generation = ? AND attempt_id IS NULL AND source_job_key > ?
...
func (r *Repository) FinalizeBaselineListing(ctx context.Context, expectedVersion uint64, baseline model.BaselineGeneration, checkpoint model.IncrementalCheckpoint, businessAt time.Time) error {
	if baseline.Version != expectedVersion+1 || !baseline.ListingFinalized || baseline.WorkID != "" ||
		baseline.ListingAttemptID != "" || baseline.SourceID != checkpoint.SourceID || checkpoint.Version != 1 {
...
WHERE source_id = ? AND baseline_generation = ? AND attempt_id IS NULL`, baseline.SourceID, baseline.Generation).Scan(&staged); err != nil {
...
SET generation_status = ?, listing_finalized = ?, details_expected = ?, listing_attempt_id = ?,
...
WHERE source_id = ? AND baseline_generation = ? AND version = ?`,
		baseline.Status, baseline.ListingFinalized, baseline.DetailsExpected, nullableString(baseline.ListingAttemptID),
		nullableString(baseline.MaterializationCursor),
