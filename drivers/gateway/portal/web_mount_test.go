...
func TestOptionalWebMountServesItsOwnIndexAndAssets(t *testing.T) {
	...
	portal := New(Config{ContractVersion: "5", WebMounts: map[string]fs.FS{"/society/": assets}})

	redirect := httptest.NewRecorder()
	portal.ServeHTTP(redirect, httptest.NewRequest(http.MethodGet, "/society", nil))
	if redirect.Code != http.StatusTemporaryRedirect || redirect.Header().Get("Location") != "/society/" {
		t.Fatalf("mount redirect status=%d location=%q", redirect.Code, redirect.Header().Get("Location"))
	}

	for path, want := range map[string]string{"/society/": "laboratory", "/society/app.js": "working-surface"} {
