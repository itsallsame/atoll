...
const commands=new Set(['install','doctor','resume-save','resume-list','resume-get','profile-save','profile-get','resume-delete','task-delete','maintenance','submit','status','report','answer','pause','stop','skip','resume','continue']);
...
let client;
try{
 if(!commands.has(command)||configIndex<0||!args[configIndex+1]||args.length!==3)throw Error('usage: cvmax <install|doctor|resume-save|resume-list|resume-get|profile-save|profile-get|resume-delete|task-delete|maintenance|submit|status|report|answer|pause|stop|skip|resume|continue> --config <private-client.json>');
 ...
 else if(command==='profile-save')result=await client.saveProfile(input);
 else if(command==='profile-get')result=await client.getProfile();
 else if(command==='resume-delete')result=await client.deleteData({kind:'resume',id:input.resumeId});
 else if(command==='task-delete')result=await client.deleteData({kind:'task',id:input.taskId});
 else if(command==='maintenance')result=await client.invoke('maintenance',{});
 ...
 else if(command==='resume')result=await client.resume(input)
