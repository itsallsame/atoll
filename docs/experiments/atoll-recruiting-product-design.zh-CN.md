...
- 浏览器对 XHR/fetch POST 仍在到达 origin 前阻断，但可把满足公开查询安全子集的请求记录为 `public_query_evidence`：只保留规范公网 URL、固定 JSON object、公开白名单 header 和 body SHA-256，最多 20 条且编码后合计不超过 64 KiB；Cookie 被丢弃，Authorization/Proxy-Authorization、敏感 JSON 字段、重复 key/header、超量或非 JSON 请求不形成候选。该证据没有执行和发布权限。已物化候选 Source 可由 Agent 调用 `recruiting.recipe.prepare`，但 Agent 只提交由已验证响应证明的语义映射（集合、岗位身份、详情地址/模板、活动时间、置顶标记与分页字段）；Actor 从冻结的请求证据确定性构造 ABI、transport、User-Agent、安全预算、倒序/更新置顶契约、增量边界和重叠窗口，执行共享 ABI 校验后创建不可覆盖的 content-addressed `recipe://` Resource。公开契约不再允许 Agent 拼完整 Recipe Spec。Recipe/Source 验证成功后，发布命令同样只接收 `validation_work_id`，由控制面从冻结执行和权威 Artifact 表派生 Recipe、Assignment fence、Checkpoint 策略与证据集，避免模型猜测内部 Artifact ID。用户不填写 Recipe JSON 或内部 ID；
- 面向用户的自然入口是 `recruiting.onboarding.begin/status/advance/materialize`：用户只提供公司名；系统解析 Company、启动或恢复 Mission。`advance` 每次只根据数据库中已提交的证据推进一个安全步骤：没有 Probe 时从已确认官网创建初始 Browser Work，有未验证公开查询时创建 HTTP Verification Work，有已验证响应时创建引用 Artifact 的后续 Browser Work。Probe、Verification、Work 和 Stub ID 均由控制面确定性生成，用户和 Agent 都不拼内部 ID、请求或响应正文；异步 Work 未完成时只返回等待，不重复创建。Mission 完成后，`materialize` 才将全部“已验证且已分类”的 ListURL 幂等、原子地登记为候选 Source。已物化的候选 Source 若还没有 Listing Assignment，`advance` 会启动下一代、有界的来源初始化证据 Mission，并优先逐个探测候选 Endpoint；证据任务仍停在 Recipe 推导/审批门前，不能因为 URL 已知就猜测请求、Extraction 或分页。底层 `recruiting.source.add` 与逐对象 Deep Discovery 命令仍保留给明确的结构化运维场景；自然语言主流程不依赖它们拼装结果。
...
新 Source 的首次 Detail Recipe 绑定不是 rollout：rollout 只替换已有 Assignment，不能用测试种子或数据库直写补齐首次绑定。冷启动顺序固定为：候选 Source 用候选 Endpoint 验证首个 Listing Recipe，但不发布 Source；Recipe 审批后再执行 Source 四维校验并发布 Listing Assignment；首轮 baseline 列表安全完成后，从其成功 Attempt 的 staging 中确定性选择一条真实岗位，只物化 `detail_pending` 的样本 Job，不推进 baseline 游标、不创建详情 Work；首个 Detail Recipe 用该冻结 Job/URL 验证，审批后再由公开 `recruiting.recipe.assign` 绑定。绑定完成后正常 baseline materializer 重读同一 staging 行并创建 generation-1 Detail Work。公开 `recruiting.recipe.assign` 只允许在 Source 已完成 Listing 校验、当前 Listing Assignment 精确匹配且尚无 Detail Assignment 时，绑定一个同 scope、active、v1 的 Detail Recipe；Source version、首条 Assignment/历史、命令回执与 `source.detail_recipe_assigned` 事件在一个事务内提交。第二次“首次绑定”必须拒绝，后续变更统一走验证后的 rollout/rollback。
...
网络发现也不能假装一次浏览必然看见全部 API。若目标岗位查询只有在另一个被阻断 POST 成功后才会发起，首个 Probe 只能保存已实际观察到的前置公开查询；不得为“继续页面”自动放行未知 POST。Browser Broker 已实现“候选查询独立 HTTP 验证→以已验证响应作为本地 stub→下一次 Browser Probe”的安全推进链：Stub 必须逐字节匹配规范 URL、公开 header、JSON body/body hash，只接受 2xx JSON、单项不超过 1 MiB、每 Session 最多 10 项且合计不超过 4 MiB；每项最多 fulfill 一次，只返回 Content-Type、no-store 和绑定页面 origin 的 CORS header，不透传 Set-Cookie。fulfill 发生在 Chrome 本地，既不增加允许写请求数，也不会把未知 POST 发给网站；未命中或重复请求仍被阻断。

该链的生产控制权属于 Recruiting Actor，而不属于页面或 Executor。用户或 Agent 只能从已完成、同一 Mission 的 Probe 中选择 endpoint/body hash，调用 `recruiting.deep_discovery.public_query.verify`；Actor 消耗冻结的 Mission 操作预算，创建 `deep_discovery_public_query` Work，并仍由唯一 Executor class 的 `http.fetch` capability 执行。HTTP Driver 复用 robots、SSRF、origin 限速、熔断、零重定向和响应大小边界，响应 Artifact 必须先写入 Atoll Resource，随后结果事务才以 Mission/Probe/Work/Attempt/request/Artifact fence 完成 Verification。下一次 `deep_discovery.browser.observe` 只接受这些已完成 Verification 的 ID；offer 从数据库解析不可变 Artifact 引用，Executor 回读并复核内容哈希后才构建本地 Stub。页面不能提交响应正文，Executor 也不能把另一个 Mission、请求或 Artifact 的内容替换进来。Broker attestation 绑定 endpoint、请求 body hash 与响应 Artifact hash，控制面拒绝任何未由本次 Probe 冻结的 fulfill 声明。

真实字节页面已证明：本地回放独立 HTTP 验证的 `config/job/filters` 响应后，Browser 能继续观察并阻断 `search/job/posts`。覆盖审计继续把尚未观察到的岗位查询记为 gap，而不是把前置配置接口误当成岗位列表 Source；验证完成也只说明该前置请求可安全复用，不会自动把它发布成 Source 或 Recipe。
...
候选正文必须先由操作者、Agent 或插件保存为权限受控的 `recipe://`/`artifact://` Resource，再通过 `recipe.propose` 进入领域系统；用户页面仍只需用自然语言表达业务目标，不填写 Recipe JSON 或内部 ID。Listing/Detail 提案以 Source 为并发目标，使用 `expected_version` 和 Endpoint revision 绑定来源；Discovery 提案则以 Company 为并发目标，用 Company version 与官网 URL 绑定来源，不能为了复用 Source 围栏而创建假 Source，也不携带 Endpoint revision 或 Extension Capture。Actor 自己读取 Resource，并与 Executor 共享同一个 256 KiB、拒绝未知字段/尾随 JSON 的 ABI 解码器。客户端只能声明 Resource 引用及预期 content hash，不能提交 scope、contract hash、状态或 Assignment；Actor 从实际 Spec 计算规范 content hash，从 transport、Request、Extraction、pagination 和 Listing boundary 计算兼容性 contract hash。事务只创建 immutable draft、command receipt 和审计事件，不改变 Company/Source/Assignment/Checkpoint。插件路径仅用于已知 Source 的 Listing/Detail，并另外提交严格有界的 Extension Capture Resource；Actor 将其中的 capture identity、消息信封操作者、Source/version、active Endpoint revision/URL、Candidate hash 与 Recipe Resource 逐项交叉校验，并把 trace/evidence 引用作为独立不可变 Proposal 事实和 Draft 原子保存。`recipe.inspect` 可审计其来源；该事实没有激活或生产写入权限。开发预览插件通过 loopback-only、随机令牌配对的 Recruiting Bridge 使用普通用户现有协议，不放宽 Atoll 的同源 WebSocket 防线，也不新增 Worker/Actor；声明式 HTML Listing 与 Detail 的真实 DOM 捕获内核和 Bridge→Atoll 链路已验证。Listing 页面由 active Endpoint 围栏并从其推导 scope；Detail 页面必须绑定已属于该 Source 的 Job ID，并由 Bridge、Actor 和提交事务逐层复核当前 Job version/detail URL，再从详情页推导 scope，避免把列表 Endpoint 与详情样本 URL 混为一谈。

候选 Recipe 的验证不是一个直接改状态的管理动作。常规 `recipe.validate` 选择已 ready 的真实 Source，把候选版本、当前生产 Endpoint、Company/Source version 和一个“当前 Assignment version+1”的拟议 Assignment 冻结进独立验证 run。首个 Listing Recipe 是受限例外：仅当 Source 为 candidate、只有 candidate Endpoint、没有生产 Endpoint/Listing Assignment，且提案的不可变 provenance 精确匹配 Source version、Endpoint revision/URL 时，才可冻结 assignment version 1 执行；验证全程不发布 Source。首个 Detail Recipe 同理只接受 initializing Company、ready Source、无 Detail Assignment，以及可反查到已完成 baseline staging 的 `detail_pending` 样本 Job。所有验证均经统一 Work/Attempt/Permit/capability dispatch 交给同一个 Recruiting Executor。拟议 Assignment 只作为执行 fence，不写入当前 Assignment 或历史；验证结果只保存 Artifact 和客观质量证明，不推进业务游标或发布详情。验证执行成功与候选质量通过是两个不同结论。
...
Browser 与 Artifact 故障也复用同一个领域恢复原语，不引入 Browser 专用调度器。BF0 将真实 Chrome 放在执行 daemon，把 File Artifact 放在另一个已认证 daemon；当 Browser Attempt 已 running、origin 已接受文档请求且尚无证据时杀掉 provider。旧 Attempt 按 TTL 过期，来源 dispatch 被持久收口，`attempt_recovered` 创建新 dispatch 和不同 Attempt；provider 恢复后才接受一份 DOM、effect trace 和 BackfillOutput。Permit、dispatch、receipt 和 Artifact fence 都保持原有边界，Server 重启后分别回读两类 Resource。该结果证明“请求已经产生但证据存储暂不可用”不会生成无证据终态，也说明恢复能力属于统一 Work/Attempt 协议而非新增 Worker 类型；它仍不代表远程对象存储、同时杀死 Chrome/provider 或第三方授权站点。

BF1 又在相同切点同时杀掉 Chrome 所在执行 daemon 进程组与 provider，保留 Server、Recruiting control 和 MySQL 在线。此时恢复不等待失联 Executor 自报失败：设备消失和 incarnation fence 使控制面收口旧 Attempt、来源 dispatch 与 Permit，并持久化新的恢复 dispatch；两台设备恢复后，新 incarnation 才能用不同 Attempt 执行。实测旧 Attempt 零 accepted Artifact，最终只有新 Attempt 的一份 DOM、trace 和 BackfillOutput，重启后两类 Resource 仍可读。因此进程联合故障仍由统一状态机解决，不增加恢复 Worker；它关闭本地双进程切点，但不外推到 Server/MySQL 同时故障、远程存储或第三方站点。

BF2 把故障边界移到 Atoll Server：真实 Chrome 文档请求已经进入受控 origin 且尚无 Artifact 时杀 Server，但保持执行 daemon 的同一 OS 进程存活。第一次运行观察 211 秒后，daemon 虽已重连，旧 Attempt 仍停在 running；根因是 Server 可能在一次性 reconcile fire 已出队、但后继 timer ID 尚未写回 Actor state 的窗口退出，启动代码又因旧 ID 非空而没有补链。修复只位于 Recruiting Actor：每次 Actor body 启动先持久化新的周期 reconcile timer ID，再尽力取消旧 timer；迟到旧 fire 仍由当前 ID fence 拒绝。每日 cutoff/work/close timer 不采用该替换规则，因为它们的精确 deadline/payload 必须跨越业务截点保存。权威复跑中原 daemon 以同一 Actor 身份和新 incarnation 重连，旧 Attempt expired 且零 accepted Artifact，新 Attempt 形成唯一 DOM、trace 和 BackfillOutput，最终重启后 Resource 可读；没有增加 Worker 类型，也没有修改 Atoll core。该证据只关闭在途 Browser 请求期间的 Server 单点中断，不代表跨每日 cutoff 的故障、Server/MySQL 同时故障、远程存储或第三方站点。

BF3 单独验证数据库短时不可用，不把数据库停顿误建模为抓取失败。测试在 Browser Attempt 已 running、真实 Chrome 请求已到 origin 且零 Artifact 时暂停非 root MySQL 8.4 容器 7.048 秒，Server 和执行 daemon 进程始终存活；页面完成后的 Chrome 子进程寿命不作为本门断言。数据库恢复后仍由原 Attempt 提交唯一 DOM、trace 和 BackfillOutput，网站文档请求保持一次，既没有创建替代 Attempt，也没有产生 `attempt_recovered` dispatch；随后正常 Server 重启仍可回读两类 Resource。这证明连接保持下的有界 MySQL stall 可以由现有数据库客户端和统一 Attempt 协议收口，无需新增 Worker、业务状态或 Atoll core 改动。它不证明 TCP 断连重建、主从切换、远程数据库、长停机或 Server/MySQL 联合故障。

BF4 进一步关闭“连接本身已经失效”的不同切点。测试透明代理关闭 Server 到 MySQL 的全部既有 TCP 连接并拒绝新连接 7.003 秒；首次未修复运行中，网站虽只访问一次，Executor 却把控制面返回的 `internal_unavailable` 当成终态，导致零 Artifact 的 Attempt/Work 持续 running，并等待来源 dispatch 的 15 分钟确认期限。修复仍只在 Recruiting Executor：结果的 command ID、Attempt/incarnation、operation 和 payload 全部保持不变；模糊传输结果立即重放一次，明确的 `internal_unavailable|channel_unavailable` 在 `control_wait_ms` 内以 250 ms 起步、2 秒封顶的指数退避重送，业务拒绝和协议错误绝不重试。单项与最多 32 项的 batch 结果共用该规则。权威复跑只访问网站一次、只保留原 Attempt，并提交唯一 DOM、trace 和 BackfillOutput；没有增加 Worker 或修改 Atoll core。该证据仍不代表主从切换、数据库进程死亡、数据丢失、远程网络或超过控制等待窗口的长停机。
...
批量优化也必须服从证据。整批身份预检将 claim/result 的归属检查从每 Attempt 一次查询收敛为最多 32 个主键的一次查询，并让所有 payload、重复 ID 和 incarnation 在首个成员提交前通过；这提高了错误批次的拒绝边界，但 H2 实测只改善 0.85%，不能宣传为容量突破。把每次 wake 连续处理批次数从 4 提到 8 的 H3 对照只改善 0.02%；早期只把 1—32 个 claim 合并为一个事务的 H2 对照也只改善 0.58%，两项当时均已撤回。这不等于永远禁止有界事务，而是不能以单点收益为依据扩大锁范围。后续每日路径的实现同时合并 Offer/Claim/Result 事务、聚合 Permit 计数并消除 Detail 重复 scope fence 查询，而且为异常批次保留原子回滚与逐项兼容路径；完整 D2/D3 和故障回归证明组合收益后才接受。每个 Attempt 的状态、receipt、Permit、Artifact 和失败处置始终独立。

第一项查询计划优化只收窄 supply-batch 的候选发现：因为该协议只允许 Detail 和历史回填，候选 SQL 不再评估其他执行类型的大 OR 分支；候选锁定后仍执行原有全部授权、scope、fence、预算和持久化。两个 H2 均为 72.620 秒（13.77/s），H3 为 474.554 秒（10.54/s），分别比原基线改善 2.63% 和 3.18%。这说明查询形状确实是成本之一，但 H3 仍比 400K/8h 的算术速率低 24.14%；下一阶段要继续测量并优化逐 Attempt fence/状态/result 路径，或在不改变公开领域权威的前提下验证外部水平部署，不能把小幅本机收益外推成生产结论。
...
因此新增独立的每日 Detail 容量旅程，而不再用 Backfill 作替代证据。真实 Atoll durable timer 先按 cutoff 建立一份 SourceOccurrence 和 Listing Work；生产 HTTP Listing Driver 在受控无出站 origin 上按 500 项分页，到达旧活动时间边界后提交一次 Checkpoint，并从每页新 Observation 物化 Detail Work。同一批处理 Executor 收到不属于批量安全集合的 Listing wake 时，空批次不提前完成 dispatch，而是回退到原单项 offer；页面接受后只按当时可选 Detail Executor 数追加紧凑唤醒，不随岗位数逐项发送。D0/D1/D2/D3 分别完成 20、200、1,000、5,000 个新岗位，其中 D3 是 10 页 Listing、5,000 个 Detail GET/Attempt/JobDetailVersion/response Artifact，20,480,000 字节逐 Resource 校验，Permit 归零、执行 dispatch 全部交付并通过 Server 重启恢复。首个基线 D2 为 77.079 秒（12.97 Detail/s），D3 为 423.655 秒（11.80 Detail/s）。随后只在 Recruiting 扩展内把批量安全 Detail 的 Offer、Claim、Result 收敛为最多 32 项的有界事务快速路径：共享预算维度只锁定并增减一次，但每个 Attempt、Work、Permit、receipt、Artifact、结果和幂等键仍单独持久化；异常批次完全回滚后走既有逐项语义，Listing、Browser 和多结果任务不参与。复测 D2 为 60.645 秒（16.49/s），D3 为 351.175 秒（14.24/s），相对 D3 基线提高 17.1%，按该本机受控速率执行 400K 约需 7.80 小时，越过 8 小时算术参考。测试在全部详情完成后以不可变窗口末作为逻辑时钟结算日报，并明确证明未来日期的关闭事件不会被 reconcile 提前投递。该证据关闭“每日 Listing 到 Detail 的 5,000 项真实数据面”及本机算术吞吐参考，但不等于实际执行 20K 同时 Listing、400K Detail、第三方限流、远程 Artifact、多分片或长期生产 SLO。
...
正式运营入口使用 `/staircase/` 前缀，并复用 Atoll 的同源登录会话、`/ws` 协议、频道成员目录和 `agent.ask`，不在页面中直连 MySQL，也不创建第二套业务 API 或权限体系。Recruiting Actor 通过只读词 `recruiting.console.snapshot` 提供有界、一致的业务投影；公司和 Source 详情继续使用既有领域查询词。系统健康与容量进入对应页面时按需读取，避免拖慢业务首屏。

信息层级固定为三层：总览先回答业务是否正常、今日增量进度、六段业务流水线、待处理和数据成果；公司、今日运行、待处理、数据结果、系统状态页面再展开业务详情；内部 ID、完整状态和容量证据只进入可展开的技术证据区。人工操作统一转换成带当前业务上下文的自然语言消息，由 Agent 通过现有 Recruiting Actor 状态机推进；页面不要求用户填写 Work、Source、Recipe 或版本号。

控制台不得改变领域事实口径：每日运行是全部 active Source 的顶部增量扫描，不是岗位全集差分；Source Discovery 只在接入、失效修复或复核时运行；公司与岗位列表 URL 是一对多；专项、校招、社招和综合来源分别展示；岗位删除仍不在范围内。移动端保留相同信息和对话入口，导航折叠后不可被键盘或读屏误访问。
