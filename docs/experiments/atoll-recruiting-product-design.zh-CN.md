...
字节跳动校招列表的 2026-09-23 真实浏览器复核进一步明确了验证口径：页面点击“下一页”会向同一路径 `/api/v1/search/job/posts` 发送多种查询。Listing Recipe 必须冻结主列表请求体中的业务筛选条件，仅允许 Probe 中由点击实际证明的分页字段变化；本次主列表的 `offset` 依次为 0、10、20，每页返回 10 个岗位。三页的首条 `publish_time` 分别为 1790093477234、1767890605243、1789390422676，不能按发布时间认定倒序。该 Source 的可靠增量资格仍须基于实际排序字段或稳定顺序的独立验证；仅能成功翻页和解析岗位，不足以证明可以按旧边界安全停止。
...
生产控制权仍属于 Recruiting Actor：Browser Work、Attempt、Permit、Artifact、结果 fence 和 Mission 预算沿用 Atoll 原语；页面只能产生网络事件，不能提交伪造响应。Executor 必须先持久化 DOM、每个 JSON response 和 trace，控制面再原子接收 Probe 结果。Recipe 准备只读取同 Mission、已成功 Probe 的 response Artifact，并用 method + endpoint path 固定业务查询；运行时临时签名永不成为 Recipe identity。真实 `https://jobs.bytedance.com/campus/position` 已验证该链可在取得 `/api/v1/csrf/token` 后捕获 200 的 `/api/v1/search/job/posts?...&_signature=...` JSON response，不再需要独立 HTTP 验证或 Stub 回放。
