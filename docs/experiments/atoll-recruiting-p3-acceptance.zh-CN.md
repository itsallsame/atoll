...
- opt-in 真实网站进程验收 `scripts/recruiting-live-e2e.sh` 使用普通 `recruiting-live-operator` 的 Home Channel、真实 server/daemon、Recipe KV、Artifact File 和非 root MySQL。测试先让 daily timer 原子提交 occurrence/Work/pending dispatch，在首次投递前强杀 server；重启、重新登录并确认 daemon Executor 恢复 present 后，同一 Recruiting Actor 从 MySQL 找回 dispatch，完成 HTTP Executor→Greenhouse 公共 Job Board API→分类结果→authenticated completion。2026-09-08 实测 Work=`waiting_human`、Attempt=`failed`、failure=`quality_rejected`、delivered dispatch=1；真实列表未满足活动倒序契约，因此证据落盘但 Checkpoint 未推进；
