...
- Listing 批量 canary 已接入既有 reconcile timer：逐项 apply 使用持久业务时间和确定性命令，随后创建真实 Source validation Work/dispatch；成功证据自动经过 Source publication 闸门，成员全成功才由数据库重算推进 wave/完成父 Work。跨 Repository 重建的非 root MySQL 合同已证明 Assignment 不重复。Detail canary、显式 rollback/resume 和真实进程 E2E 仍待完成，S22 仍不关闭；
