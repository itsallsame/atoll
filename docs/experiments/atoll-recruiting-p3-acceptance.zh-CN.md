...
ATOLL_E2E_BIN=$PWD/bin go test -count=1 ./e2e \
  -run 'TestRecruiting(P0Journey|CompanyAndSourceControlUsesMySQLAcrossServerRestart)' \
  -v -timeout 240s
...
- Job/DailyRun 的 list、System/Capacity 查询，以及 Work/DailyRun 修改控制词；Source validate 当前只进入 `validating`，验证 Attempt 的接受、契约证明和原子发布仍属于后续纵向切片；
