...
- Listing 批量 canary 已接入既有 reconcile timer：逐项 apply 使用持久业务时间和确定性命令，随后创建真实 Source validation Work/dispatch；成功证据自动经过 Source publication 闸门，成员全成功才由数据库重算推进 wave/完成父 Work。跨 Repository 重建的非 root MySQL 合同已证明 Assignment 不重复。失败 wave 会暂停父 Work，公开 resume 只有在旧失败 Work 终结后才原子重置固定 wave，且 applied 成员不重复切换；恢复代次会创建新的 Work/run/command identity，并显式登记该代 Source version。纵向合同已覆盖质量失败、Source invalid、暂停、恢复和第二代验证成功。Detail canary、显式 rollback 和真实进程 E2E 仍待完成，S22 仍不关闭；
