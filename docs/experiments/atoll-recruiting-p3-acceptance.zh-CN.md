...
- 主动 incarnation 失效已复用 Atoll 公共 `system.member.list`：只把 substrate-owned `present/uptime` 作为加速信号，不信任 Executor 自报 heartbeat。控制面只跟踪成功 offer 的 authenticated concrete Actor ID，并在重启时从最多 10,000 个活动 Attempt 恢复观察集；首次 present、离线、catalog 消失、重新上线及 uptime 回退均使用保守绑定时间，只回收严格属于旧绑定的 Attempt。每 tick 成员 sweep 和 Attempt 回收共享有界 limit 与公平游标；回收事务同时释放 Permit、重排 running Work、写唯一事件并提前持久 dispatch。真实 Lever 测试在 stale timeout 为一小时的条件下强杀第一页后的 daemon，约 104ms 后由新 daemon/new Attempt 从第一页恢复。紧凑证据见 `evidence/recruiting-live-presence-recovery-20260911.json`；
...
- `recruiting_recovery_test.go` 已闭合应用 outbox 与 Atoll ledger 的完整重启切点：测试先让稳定领域 event ID 写入真实 Channel SQLite，再故障注入“ledger append 已完成、MySQL delivery checkpoint 丢失”，强杀并重启 Server 后重复 reconcile。MySQL 恢复 delivered，而 Atoll `messages` 仍恰有一行相同 event；同一重启还验证 Company 命令与唯一 daily production recovery 头稳定重放，历史 DailyRun 继续为 `completed_with_exceptions`、version 与 `uncovered=1/recovered=0` 不变。紧凑证据见 `evidence/recruiting-server-recovery-ledger-dedup-20260911.json`；
...
- 公司导入已进入同一控制面和同一个 `recruiting-executor` class：输入正文只存在 File Resource，命令保存 opaque ref、原始字节 SHA-256、schema/policy version；`company.import` capability 不申请网站 origin/Profile BudgetPermit，也不新增 Worker 类型。CSV 重复项和行错误形成 `ready|skipped|waiting_human` 预览 disposition；结果 envelope 最多 500 项，以 Attempt/incarnation、Work acceptance、batch version 和连续 sequence 共同 fence。非 root MySQL 8.4 合同覆盖两段预览分片、稳定 command replay、错误 sequence/CAS、错误 completion hash 全事务回滚、控制面重算 hash、父 Work `waiting_human(preview_ready)`、seek pagination、精确 hash/version 确认和有界 apply 多页续跑。apply 每项独立事务创建 Company/子 Work/outcome，数据库既有公司形成 `waiting_human` 而不回滚成功项。公开 `company.import.item.resolve` 只允许精确版本的 `retry|skip`：重试要求先由正常 Company 运维消除外部冲突，无效源行只能明确跳过而不能改写 immutable preview；事务从全部 item 事实重新聚合，最后异常消失时才完成父 Work。MySQL 合同验证冲突未修复时零副作用、并发命令收敛和最终重聚合。Executor 单测覆盖 File Resource 读取、原始哈希不符 fail closed、重复/错误行、改变 chunk size 后续传及 apply envelope 校验。真实进程 E2E 由普通运营员通过 Portal 上传 daemon File Resource，完成 preview→分页审阅→confirm→多页 apply→Company update→冲突项 retry→无效项 skip；最终为 3 成功/2 跳过/0 等待人工，父 Work 完成，输入 Resource 字节不变。紧凑证据见 `evidence/recruiting-company-import-human-resolution-20260911.json`；
- Recipe 批量灰度的公开控制入口已接通：普通用户提交最多 20,000 个 Source 的严格版本化 JSON Resource，Actor 复核原始 hash 后以最多 500 项的 Repository 短事务构造确定性预览；最终 hash 绑定 schema/policy、目标 Recipe 和全部 Source/Assignment fence。`batch.get/items` 支持审阅，`batch.confirm` 只打开 canary 并启动父 Work，`batch.cancel` 只允许没有应用事实的批次并释放活动 scope。真实 server、KV Resource、非 root MySQL E2E 已覆盖创建重放、两页审阅、确认、取消和 server restart 后重放；逐项切换、验证、wave reconcile 与 rollback 的当前证据见后续条目；
- Listing 批量 canary 已接入既有 reconcile timer：逐项 apply 使用持久业务时间和确定性命令，随后创建真实 Source validation Work/dispatch；成功证据自动经过 Source publication 闸门，成员全成功才由数据库重算推进 wave/完成父 Work。跨 Repository 重建的非 root MySQL 合同已证明 Assignment 不重复。失败 wave 会暂停父 Work，公开 resume 只有在旧失败 Work 终结后才原子重置固定 wave，且 applied 成员不重复切换；恢复代次会创建新的 Work/run/command identity，并显式登记该代 Source version。批量 rollback 会逆序恢复整个已发布前缀到各成员冻结的旧 Assignment，Listing 成员必须重新验证；rollback 失败也可暂停、人工修复和显式恢复。纵向合同已覆盖两次目标质量失败、Source invalid、暂停、恢复、旧 Recipe 回滚验证、Assignment old→target→restored、Source ready 和批次 `rolled_back`；
- Detail 批量 canary 已复用同一批次、成员和 Attempt 控制面，但验证保持业务特化：切换 Assignment 后，从 Source 的稳定 `available` Job 选择真实样本，以 active 目标 Recipe 创建 evidence-only `recipe_validation`；Company/Source/Assignment/Recipe/Job 和 Permit 全部参与 fence，成功仅落 response/trace 与质量摘要，绝不写 Job 或 DetailVersion。候选验证与 rollout 验证具有显式 mode，旧状态 JSON 仍按 candidate 兼容读取。整批 rollback 同样恢复完整已发布前缀，并要求旧 Recipe 产生新的 evidence-only 样本成功证据。非 root MySQL 纵向合同跨 Repository 跑通旧 Recipe 真实详情→批量切换成功，以及第二目标验证失败→人工终结 Work→整批回滚→旧 Recipe 验证→`rolled_back`；Assignment 历史单调且样本岗位事实不变。真实 Server/daemon/统一 Executor E2E 又动态选择 MongoDB Greenhouse 当前公开岗位：两个 Source 以 1 canary + 1 wave 顺序完成；第二批坏 Recipe 真实失败并停止扩散，普通用户终结失败 Work、显式 rollback 后由旧 Recipe 重新取证，最终 `rolled_back`。Job 与 DetailVersion 前后逐字段不变，两个 Source 的 Assignment 历史分别为 4/2 条；
- 20,000 Source 容量合同真实建立 40 个 500 项预览事务，并验证 10 canary、严格 500 项多 wave、499/500 成功不扩散、500/500 才推进。它发现并修复第二 wave 的父 Work event version 冲突；进度事件现归属 Batch。仍运行或退避的 validation 不再反复占 reconcile 配额，终态会重新进入工作清单；多个活动 Batch 按 tick 总预算均分。隔离 MySQL 合同 67.034 秒通过。结合上述真实进程 canary/wave/失败回滚旅程，S22 的自动化退出证据已闭合；单次公开样本仍只证明当次只读执行，不外推第三方站点长期可用性；
...
ATOLL_RECRUITING_LIVE_E2E=1 go test ./e2e -run '^TestRecruitingLiveDetailRecipeRolloutBatchThroughAtoll$' -count=1 -v
./scripts/recruiting-boundary-check.sh a94d2b8d
```

## 尚未完成

- 本阶段列出的控制面能力与定向故障切点已经实现；尚未完成的是在同一最终 revision 上重跑 P3 全量退出门并归档整套结果，不能用分散的历史通过记录替代。
