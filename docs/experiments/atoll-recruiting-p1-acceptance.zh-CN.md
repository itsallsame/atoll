...
- Recipe version 强制携带 ABI、opaque content ref、transport 和 required capability；SourceOccurrence 冻结最小 Listing 执行快照，并通过 `planned→queued→running→terminal` 状态机绑定唯一 Work，不能在已有 Work 后直接排除；
