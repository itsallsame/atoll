...
- Recipe version 强制携带 ABI、opaque content ref、transport 和 required capability；SourceOccurrence 冻结最小 Listing 执行快照，并通过 `planned→queued→running→terminal` 状态机绑定当前唯一 Work，不能在已有 Work 后直接排除；纯领域测试证明 queued/running occurrence 可用精确旧 Work 和版本重绑到不同的 retry Work，陈旧链接、同一 Work 和终态 occurrence 均被拒绝；
