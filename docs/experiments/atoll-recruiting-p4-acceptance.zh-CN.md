...
- Recipe 与 Artifact Resource 的 Channel 权限及重启保持已有真实 Server/daemon 证据：`TestRecruitingRecipeResourceAccessSurvivesRestart` 在两个普通用户各自的 Home Channel 创建独立 Recruiting Actor，并让二者共享同一招聘 MySQL。Owner 创建的 Recipe KV 和 Recruiting Artifact File 在 Server 重启后仍可由原用户逐字节读取，Recipe 可继续形成 draft；另一 Actor 即使知道 Source、Resource ID 和正确 content hash，也无法跨 Channel 读取二者或发起提案，且数据库没有泄漏 Recipe 或 command receipt。该边界完全复用 Atoll Resource 权限，不在招聘数据库中复制权限系统；
...
- Artifact provider 在 Attempt 已 start 后不可用时，Executor 不会发送无证据的分类失败或伪造终态，而是返回明确 provider 错误，让持久 Attempt 由统一 TTL/reconcile 恢复；该切片已有执行生命周期合同，但真实 provider 断连后的完整进程恢复仍属于 P9 故障矩阵；
...
- Browser Plan 现可声明哈希绑定的 `auth_expired_selectors` 与 `captcha_selectors`，每类最多 10 个合法 CSS selector，跨类别重复会因语义歧义被拒绝。真实 Chrome 在初次导航及每个受限动作后检查信号，命中时仍返回 DOM 供 Artifact 先持久化，再由 Browser Driver/Browser Execution Driver 端到端保留 `auth_expired|captcha`，不再误降级为 `browser_transport/unexpected_status`。确定性真实 Chrome 两类页面、Driver Artifact 和 ABI compatibility hash 的 race 套件均通过；既有非 root MySQL Profile 失败合同继续负责原子熔断、单飞 RepairIncident 和旧 Attempt 围栏；
- Broker 在导航后、DOM 完成前异常退出时，Driver 保存绑定原 Attempt 的部分 DOM Artifact，再返回稳定 `browser_transport`，不会把残缺页面交给离线 Recipe 或生成业务结果；该行为已有独立 Browser crash 合同；
- Browser crash 与 Artifact provider 同时不可用的组合已经穿过真实 Browser execution adapter 和统一 `executeOffer` 生命周期验证：Broker 返回部分 DOM 后崩溃，Artifact Resource 拒绝 DOM 写入，随后本地 failure Artifact 写入也失败；控制面只收到 `accept/start`，不发送没有证据的 `execution.failed`，持久 Attempt 留给既有 TTL/reconcile 过期恢复。该合同不把组件内故障注入冒充 OS 级进程验收；
...
- Discovery 候选入口与执行均按真实领域归属实现为 Company 级：company target 使用 Company version 和官网 host 作为 fence，禁止伪造 Source endpoint revision、Assignment、production discovery generation 或 Extension Capture。`recipe.validate` 创建通用 `RecipeSampleValidation` 与统一 Work/Attempt，仍由同一个 Executor class 复用 Discovery Driver；结果只保存 response/trace、候选数、字段数和规范 hash。零候选是可收口的负证据但不能审批；成功审批至少要求一条候选。非 root MySQL 合同与普通用户真实 server/daemon 均断言 Company 未改变，也没有创建 Source、生产 SourceDiscovery 或 Candidate。
- Profile 修复侧已增加 loopback-only 本机 relay，而不是新增 Worker 或通用远程浏览器服务：同一 `recruiting-executor` class 以 `browser.profile.repair` capability 领取设备绑定 offer，通过独立文件令牌把有截止时间的请求交给配对插件。Profile 冻结站点专用 Browser Recipe 的精确 HTTPS endpoint、ID/version、content/contract hash 和最小记录数；Executor 在 accept 前从 Atoll Resource 解析复核，插件在精确页面执行并只返回 Recipe 身份、记录数和结论。Bridge 最多保留 8 个离线等待请求，双令牌不可复用，负向 canary 会立即返回。扩展纯逻辑、整包 Chrome、Bridge、Executor race 测试和非 root MySQL 8.4 控制合同已通过；日常 `browser.recipe` 已接入本机 Profile Provider，真实授权登录站点尚未通过，不能宣称 Browser/Profile 产品完成。
- Source 已有 Listing/Detail 独立的持久 Profile binding，不再依赖每次 Work 临时手填 Profile。每日 occurrence 冻结 Listing Profile ID，Work/Attempt 分别继承身份与版本；dispatch 精确投递授权设备，Offer 事务再次验证 authenticated Executor。列表页派生的 Browser Detail Work 使用独立 Detail binding 并唤醒其设备。migration 37、错误设备零 Attempt、正确设备领取和跨设备 Listing→Detail 的非 root MySQL 合同已通过；设备侧 provider 现已能按相同身份、版本和安全域解析凭证目录。
- 旧 listing Attempt 的迟到结果已有真实进程证据：`TestRecruitingLiveBaselineCancellationThroughAtoll` 在 Executor 的真实 HTTP 读取进行中由普通用户取消 baseline，daemon 随后仍经 Atoll Message 提交页面结果。控制面把原始 Artifact 保留为 rejected evidence，Work/Baseline/Attempt/Permit 保持取消后的围栏终态，Checkpoint、正式 Observation、Job、staging 和活动预算均为零；这不再只由 Repository 合同间接证明。

## 候选 Recipe 真实网站验证（负向证据通过）

2026-09-10 显式运行 `ATOLL_RECRUITING_LIVE_E2E=1 go test ./e2e -run '^TestRecruitingLiveRecipeValidationRejectsBadCandidateThroughAtoll$' -count=1 -v`。普通登录用户通过真实 Atoll server 创建控制 Actor 和 daemon 上的统一 HTTP Executor；测试只读访问 Discord 的公开 Greenhouse Job Board API、MongoDB 的公开 Greenhouse 详情接口和 MongoDB Careers 官网。候选 Listing Recipe 能成功请求、解析并产生 page/trace Artifact，但该实时列表不满足候选声明的 `newest_activity_desc`，因此验证 Work/Attempt 作为“执行完成”成功收口，后续审批明确返回 `quality_rejected`。Detail 候选从当前公开岗位动态选样并完成 response/trace→approve，未写 JobDetailVersion、未改变 Assignment。Discovery 候选经 Company 提案后以官网为冻结样本完成 response/trace→approve，未改变 Company version/onboarding，也没有创建 Source、生产 Discovery 或 Candidate。

这条用例不是把第三方数据失败包装成通过；Listing 部分验证的是审批 fail closed，Detail/Discovery 部分只证明其各自冻结样本，不等于 rollout 或生产 Source Discovery。单次实时列表不能证明 update-retop，且测试没有将该 Source 标为可生产增量。紧凑证据见 `docs/experiments/evidence/recruiting-live-recipe-validation-20260910.json`。
...
同日又用官方 Chrome for Testing 153.0.8010.36 执行真实插件整包门：Chrome 从仓库 Manifest V3 加载后台 Service Worker，测试打开实际 popup、填写 Bridge/Profile 配置并点击配对；只有 UI 显示连接成功、Bridge 保存精确 Profile 定向连接，且同一目录的每日 Runner 租约被阻塞，测试才通过。该门不注入脚本替代 popup/background，也不访问第三方站点。紧凑证据见 `docs/experiments/evidence/recruiting-extension-bundle-20260911.json`。
...
- 本地确定性站点已覆盖 redirect、空结果、字段/JSON 异常、429/403/404/5xx、慢响应/超时、响应超限、robots 拒绝、Browser POST/popup/download/跨源导航、selector 缺失以及 Recipe 声明的登录过期/验证码信号；Browser crash 与 Artifact provider 断连的组合已通过真实 adapter 生命周期合同，仍需提升为强杀 Chrome/File provider 的 OS 级组合进程切点；
