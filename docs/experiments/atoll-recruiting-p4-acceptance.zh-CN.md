...
- Browser Broker 的 Chromium/CDP 实现、OS/container 级隔离，以及插件整包在 Chrome for Testing/Chromium 中的自动加载点击门；开发预览插件、本机安全 Bridge、真实 DOM 捕获内核及 Extension Proposal→持久 Draft 已接通，Listing 候选完成真实负向验证，Detail 候选完成隔离 MySQL 与真实站点的 evidence-only 验证及审批；
- 本地确定性站点的全部异常矩阵；
