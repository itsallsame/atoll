...
- 候选不再需要测试或操作者直接写 MySQL：普通用户可先把声明式 Spec 保存为 Atoll Recipe Resource，再以 `recipe.propose` 让 Actor 验证 hash/ABI/Source fence 并创建 draft；同一 server 旅程覆盖错误 hash 拒绝、成功提案及重启重放。浏览器 Extension Capture 的完整证据封装仍待接入。
...
- Recipe 运维 E2E：`TestRecruitingOperatorQuarantinesAndRollsBackRecipeThroughServer` 使用普通登录用户、真实 Atoll server 和非 root MySQL，完成 Detail rollout、inspect、quarantine、历史回滚及 server restart 后的命令重放；仓储合同同时证明 quarantine 不改写当前 Assignment/历史，回滚把历史 v1 追加为当前 v3。
