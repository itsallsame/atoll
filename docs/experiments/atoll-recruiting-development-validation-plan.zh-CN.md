...
HTTP response capture 容量进展（2026-09-12）：新增 `recruiting.http-capacity-workload.v1`、`make recruiting-http-capacity`、受控 origin fixture 和进程级 `TestRecruitingHTTPResponseCapacityThroughRealDataPlanes`。脚本把 origin 放进 Docker `--internal` 无出站路由网络，并给它只在该网络内可达的公网形状地址；生产 HTTP Driver 未增加 private-network 测试开关，仍执行地址安全、robots、terms、GET-only、Recipe、预算和 circuit 规则。H1 的普通用户确认 200 项 `live_refetch` Backfill，四个同 class `http.fetch` Executor 各只读取一次 robots 并完成精确 200 个 Job GET；origin 记录 200/200 请求。每个 4 KiB response 都先写 daemon File Resource 后才提交结果，200 response Artifact、Attempt、Item 与 BackfillOutput 全部成功，819,200 字节逐对象回读且 SHA-256 metadata 一致，Permit 归零；4 条紧凑初始唤醒与 200 条 capacity-release dispatch 全部 delivered，领域 event 全部在 ledger 各存在一次。preview 286 ms、业务完成 37.351 秒（约 5.35 response Artifact/s）、outbox 排空 37.725 秒；测量点 ledger 3,400 条消息、2,377,017 payload bytes、310 条 event，Server 重启恢复 3.726 秒。它关闭受控 HTTP JSON response capture 的本机数据面基线，不代表第三方网络、真实延迟/429/CDN 分布、Browser、远程 Artifact provider、H2/5 倍峰值或 L3/L4 全部执行完成；S25/P9 仍为部分完成。证据见 `evidence/recruiting-http-capacity-h1-20260912.json`。

HTTP H2 容量结果与阻塞信号（2026-09-12）：最终 H2 用同一入口把负载扩大到 1,000 Job、8 个 Executor 和每响应 16 KiB。两页各 500 项的有界物化精确产生 16 条 `work_materialized` 唤醒而不是逐 Work 消息，随后 1,000 条 capacity-release 唤醒；1,000 个 origin GET、Attempt、成功 Item、BackfillOutput 和 response Artifact 全部精确一致，16,384,000 字节逐对象回读校验通过，8 个 Executor 均被使用、Permit 归零、领域 event 全部入 ledger、重启恢复通过。preview 965 ms、业务完成 181.220 秒、outbox 排空 182.164 秒，实测约 5.52 response Artifact/s；ledger 为 15,323 条消息和 11,246,239 payload bytes。正确性与 1,000 对象数据面门通过，但从 H1 的 4 Executor/5.35/s 增至 H2 的 8 Executor/5.52/s，吞吐只提升约 3.2%，不能宣称横向扩容有效。若不考虑 Listing 和失败成本而直接按该速率推算，400,000 Detail 约需 20.13 小时；8 小时窗口至少需要 13.89/s。该外推只是定位容量差距，不是生产预测；下一步必须以 Attempt 阶段时延和消息/Resource 指标定位瓶颈，并依次验证缩短事务、减少进度往返和紧凑 batch envelope，仍不得修改 Atoll core 或凭空增加 Worker 类型。证据见 `evidence/recruiting-http-capacity-h2-20260912.json`。
