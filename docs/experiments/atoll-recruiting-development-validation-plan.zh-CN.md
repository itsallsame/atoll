...
执行状态：完成。显式 Source Discovery generation、受 Recipe/Company/Attempt fence 约束的一页有界真实执行、证据 Artifact、0/1/N 候选分页，以及逐候选接受/拒绝已经进入 Recruiting 扩展；接受候选会在同一事务创建 `candidate` Source，跨 Company 的规范 Source key 冲突会完整回滚并交给人工判断。零候选结果会在确认公司没有非归档 Source 后，把 Company 与 Discovery/Work 在同一事务收口为 `blocked_no_sources`；已有其他 Source 时不误判，命令重放不重复推进。MongoDB 公共 careers 页面已经通过普通用户、真实 server/daemon、非 root MySQL 和 HTTP Executor 完成负向校准纵向验证，实际数据违反活动倒序契约时 Source 正确进入 `invalid`，且不产生业务事实。正向退出门则由 Women’s Aid 公开站点于 2026-09-11 完成：普通用户、真实 Atoll Server/daemon、统一 Executor 和非 root MySQL 贯通 `Company → discovery → Candidate → validation evidence → Source publish → Detail Assignment → baseline → all details → unchanged incremental`。同一快照中存在历史岗位更新后按 `modified_gmt` 重新置顶的真实样本，四项契约经 Artifact 回读后发布；17 个 Job、17 个 DetailVersion、0 exception，Checkpoint 1→2，baseline/production 命令重放均不改变事实。完整旅程按 robots 约束使用 origin 并发 1 和 10 秒间隔，实测 306.01 秒。当前生产物化路径另以 10,000 条 staging 验证为 20 个短事务和 40 个聚合 dispatch。P5 退出门据此关闭；持续增量、规模和故障矩阵仍由 P6—P9 的独立退出门约束。证据见 `docs/experiments/atoll-recruiting-p5-acceptance.zh-CN.md` 与 `docs/experiments/evidence/recruiting-live-qualified-wordpress-20260911.json`。

进展补充（2026-09-13，Deep Discovery v1）：公司入口发现从单页 `Source Discovery Recipe` 前移为 Agent 驱动、Recruiting Actor 掌权的长期 `DeepDiscoveryMission`。纯领域状态机固定 `scope_building → brand_expansion → site_enumeration → site_exploration → pool_detection → candidate_validation → coverage_review → completed`，每次 checkpoint 只允许停留或前进一步；冻结搜索/操作预算、证据图、覆盖矩阵、关键盲区、`waiting_human/resume/cancel` 和一公司一活动 Mission 均由 MySQL CAS 与唯一键约束。Search 只能贡献候选证据，不能单独通过最终 ListURL 门槛。真实正式节点上的字节跳动 Mission 已按七个 checkpoint 完成，持久化 40 个节点、39 条边和 3 个由 `network/detail_reverse` 交叉验证的 ListURL。

纠偏补充（2026-09-17，Snowland SOP 硬门化）：上述 v1 的阶段名称完整，但当时的完成门只检查 Evidence 类型与传感器标签，没有强制证明“真实列表页 → 点击真实岗位 → 真实详情页”，因此旧字节 Mission 及其物化结果不再视为有效验收。领域模型新增 `DiscoveryListProof`；validated ListURL 必须保存 Company/list/active-posting 三项断言、列表与详情 Artifact、sample job key、精确 DetailURL、唯一 `{value}` pattern、身份来源/路径和至少两步导航路径。Repository 在 `coverage_review` 中反查同 Mission 的 completed Browser Probe：单 Artifact 模式必须冻结 follow selector 且 final URL 等于样本详情；双 Artifact 模式必须由列表 Artifact 暴露该详情链接并由详情 Artifact真实访问。`onboarding.materialize` 再次执行同一门禁，旧的无 proof Mission fail closed。Recipe prepare 从 Source discovery generation 反查已验证 ListURL，拒绝任何与浏览器实证 pattern 不同的 `detail_url_template`。状态机同时要求进入站点枚举前至少一次真实搜索，并在覆盖复核时显式填写 social/campus/intern 的 covered、not_found_after_search 或 excluded 结论。该切片保持 Atoll 核心零修改，只加强 Recruiting 扩展的 Actor/Work/Artifact/Recipe 约束。

同日自然语言接入继续补齐 URL 业务类型和候选 Source 物化。`ListURL` 现在必须基于页面/网络证据分类为 `social/campus/intern/special/all`，具名特殊计划强制保存计划名，`unknown` 不得通过完成门；`recruiting.onboarding.begin/status/materialize` 只接受公司名并自动解析领域 ID。正式节点以真实 `agent.ask` 完成字节跳动 generation 2，保存 5 个 typed URL，并在一个有 receipt/event 的事务中物化为 5 个 candidate/healthy Source；第二次调用幂等复用。该切片关闭“发现结果只在报告里、没有进入后续生产对象”的断点，但 Recipe 自动生成/验证和 baseline 仍是 P00 剩余工作。

同轮补齐异步 `recruiting.deep_discovery.browser.observe/get`：Recruiting Actor 在一个事务内校验 Mission version、消耗一次冻结预算，并创建不可变 Probe、`deep_discovery_browser` Work、审计事件和 dispatch；既有同 class `browser.public` Executor 经 offer/accept/start/result 执行，DOM response 与 effect trace 写入 Atoll Artifact Resource。浏览器仅允许 GET/HEAD/OPTIONS、有界等待/滚动/同源跟链，结果 attestation 必须证明无写请求、表单、下载、弹窗和跨源文档导航。正式节点真实访问 `https://joinbytedance.com/search` 的 v4 回归得到 29 个规范链接，Probe/Work/Attempt 全部成功、两个 Artifact 绑定同一 Attempt、活动 Permit 为零；早先结果确认协议缺陷留下的 Attempt 也由原恢复链过期并重试成功，没有人工改库。迁移 57—59 已在指定的远程 `staircase` MySQL 应用；59 清理了 active/completed Mission 曾误存的 `waiting_reason`，全程未使用 Docker。验收见 `docs/experiments/atoll-recruiting-deep-discovery-acceptance.zh-CN.md` 和 `docs/experiments/evidence/recruiting-deep-discovery-v1-20260913.json`。

进展补充（2026-09-11，首次 Detail Recipe 绑定）：公开 `recruiting.recipe.assign` 补齐新 Source 从 Listing 校验发布到 baseline 之间的首次 Detail Assignment，避免此前只能靠测试/数据库预置的接入断点。事务要求 ready/healthy/active Source、精确当前 Listing Assignment、尚无 Detail Assignment、同 scope 且 active 的 v1 Detail Recipe，并原子写 Source CAS、首条 Assignment、不可变历史、receipt 和事件；第二次首次绑定与跨 scope 绑定均全事务拒绝。Browser Recipe 不伪造已绑定 Profile，而在响应中明确下一步先绑定 Profile；普通 HTTP Recipe 返回启动 baseline。非 root MySQL 合同覆盖成功、重放、历史和失败零泄漏；普通用户经真实 Atoll Server 完成命令、重放、Server 重启和数据库事实核对。
...
阶段性执行记录（完成前）：隔离 non-root MySQL 8.4 合同 `TestDailyIncrementalD0ThroughD6PreservesBoundaryAndRefreshInvariants` 已建立 D0—D6 连续旅程，覆盖真实 baseline 建账、必要增量与重叠、更新后重新置顶、同时间跨页、边界消失、Checkpoint 不推进、公开人工终止及因果 Retry；各提交路径同时覆盖命令重放和数据库已提交但回包丢失。D0 baseline 的进程退出由真实 Server/daemon 用例覆盖，Women’s Aid 真实源提供历史更新重新置顶证据并实测 baseline 4 页、下一次无变化增量 2 页。该阶段尚缺的 D1—D6 OS 进程笛卡尔矩阵已经由本节末尾 `0491651e` 的关闭记录补齐；机读合同证据见 `evidence/recruiting-daily-d0-d6-failure-matrix-20260912.json`。
...
进展补充（2026-09-13，D1—D6 连续进程矩阵与 P6 关闭）：固定提交 `0491651e` 将同一套连续 MySQL 血缘扩展到 D5、D6，并替代上文“D5/D6 待完成”的阶段性结论。D5 在六个新活动时间页面内找不到旧边界，Executor 必须提交分类失败并进入 `waiting_human`：Checkpoint 保持 v6、Job/DetailVersion 保持 6/7，页面进度与业务 Observation 均为零，同时保留六份原始页面证据和一份失败证据。Executor 轴在失败事务提交后强杀 daemon；Actor 轴在失败事务提交前强杀 Server，旧 Attempt 回滚、过期后由新 Attempt 重做，二者均得到同一失败结论。D6 只允许普通用户通过公开 `recruiting.work.resolve(terminated)` 和 `recruiting.work.retry` 修复同一个 D5 occurrence；重试 Work 保留 `cause_work_id`，Occurrence 原子重绑。两轴再次在第一页结果事务处执行同样进程切点，恢复后从第 1 页重扫六页并安全越过旧边界，Checkpoint 推进到 v7，新增 G/H 后最终为 8 个 Job、9 个 DetailVersion，旧失败 Work 与新成功 Work 的因果事实同时保留且 active Permit 为零。固定提交权威复跑共 12 个 OS 故障切点，Executor/Actor 连续轴分别 30.67/45.46 秒，总测试 76.14 秒、Go 包 81.163 秒；测试拥有的容器全部清理，Atoll core 零修改。结合既有 D0—D6 Repository/数据库回包丢失矩阵、D0 baseline 真实进程恢复和真实站点必要增量验证，P6 退出门关闭。证据见 `evidence/recruiting-daily-d1-d6-process-failure-matrix-20260913.json`；远程数据库切换、地域灾备、生产出站隔离和第三方授权 Profile canary 仍属于 P7—P9，不因 P6 完成而提前关闭。
...
逐场景当前状态、权威测试映射和缺口统一维护在 `docs/experiments/atoll-recruiting-scenario-acceptance.zh-CN.md`。截至 2026-09-12，25 个场景中 23 个完成；S12、S25 部分完成，因此 P8/P9 与整体产品均不得宣称完成。

执行状态补充（2026-09-12，S18 Company 合规擦除）：普通 archive 继续只做可恢复逻辑归档；物理擦除使用独立 `CompanyErasure` 状态机。请求只接受已归档 Company、全部已归档 Source、无活动 Work 和无活动 alias，冻结 Company version、按 Source ID seek 的成员/version/execution fence、政策版本、最早执行时间和滚动 preview hash；每页最多 500 项。请求人与审批人必须是不同的 human principal，不能靠同一账号更换 actor incarnation 绕过，审批再次验证 Company、成员集合和活动执行；活动请求存在时 Company/Source 均不能恢复。保留期到达后，既有 durable reconcile 先把所有 Artifact 的 object ref/content hash 固化到不依赖 Company FK 的清理清单，再按实际外键图以 48 个阶段、每事务最多 500 行删除 Company/Source/Job/Work 关联明细。Recruiting Actor 不冒充 Resource 创建者：有权用户通过 Atoll Resource 数据面删除对象，公开 `company.erasure.resource.verify_absent` 用 Stat tombstone 验证后保存清理事实。全部数据库范围和 Resource 均清理后，控制 Work 成功终结并产生稳定 SHA-256 proof；请求、冻结成员、Resource 清理记录、proof 和既有最小 command/event/dispatch 审计保留，已擦除 Company ID 永久不可复用。两个不同注册用户在共享运营频道完成 preview→逐页 reconcile→Server restart→自审批拒绝→第二人审批→Resource 存在拒绝→创建者删除→另一人确认→48 阶段擦除→proof→再次重启读取；Company 已不可查询。提交 `2fbaaab8`、`f4550388`、`11f6de13`、`057d0260`、`c735bbd8`；完整非 root MySQL 8.4 Store race 回归 228.721 秒、Actor race 3.208 秒，进程 E2E 24.43 秒，vet、diff check 和核心冻结检查通过。证据见 `evidence/recruiting-company-compliance-erasure-20260912.json`，S18 关闭。
...
执行状态补充（2026-09-12，跨执行类型的 scope fence 统一）：执行授权不再把 Company/Source 聚合 `version` 同时当作配置 fence；该版本会随 drain/resume、健康和其他运维事实推进，继续使用会误杀合法在途结果并使恢复后的旧 Work 无法重新领取。Listing、独立 ListingRun、Baseline、Detail、历史 Backfill、Source Discovery、Source validation、Listing/Detail/Discovery Recipe validation 统一由当前 active/readiness/health 状态决定能否创建新 Attempt，由独立 `configuration_version + execution_fence` 决定已经签发的 Attempt 是否仍可提交。drain 保持 execution fence，因此在途结果可结算；cancel 推进 execution fence；Endpoint/Assignment/官网等配置变化推进 configuration fence。Diagnostic 与 Recipe sample 的 fence 失败现在和 Listing/Detail/Backfill 一样，在回滚业务事务后以独立事务保留 bounded rejected Artifacts。进一步的矩阵验证发现，目标为 `recipe` 的验证 Work 此前没有持久 Company/Source 调度归属，导致共享 fence 无从生效；四个候选/rollout 创建入口现在都从事务内重新读取的领域事实强制派生 scope，并拒绝调用方伪造的冲突 scope。真实 MySQL 已逐类覆盖 Listing、Detail、Source Discovery、Source Validation、Listing/Detail/Discovery Recipe Validation、Baseline 和 live Backfill 的 drain/cancel/config 三联矩阵；被 fence 的结果只保留 rejected Artifact，不推进 page、candidate、validation、DetailVersion 或 Backfill output。暂停后恢复的 Baseline Work 仍可重新领取。完整非 root MySQL 8.4 Store 回归 236.350 秒、Actor 3.429 秒，race/vet 和冻结核心检查通过，证据见 `evidence/recruiting-scope-execution-fences-20260912.json`。逐执行类型的结果矩阵已经闭合；通过真实有界 `ScopeControlOperation(cancel)` 驱动全部业务聚合、Attempt、Permit、occurrence 的联合终态和全截点竞态仍未闭合，因此 S15/S16 继续保持部分完成。
...
执行状态补充（2026-09-12，S13 Company 官网复核与回滚）：提交 `fe003812` 增加不可变 `CompanyWebsiteRevision` 和单调 `CompanyWebsiteHead`，把官网更新、Company 配置围栏、人工 `source_relationship_review` Work、receipt 与审计事件放在同一事务；不修改现有 Source、Assignment、Checkpoint 或岗位事实。公开 `company.website.rollback` 只允许逆转当前 revision，并通过追加 revision 恢复旧 URL，不倒退 Company/configuration/head version。新的 Source Discovery 自动把当前 revision 写入 `website_revision_id`，并以对应复核 Work 为因果根；Repository 在创建事务重新验证当前 head，拒绝串版或错误因果关系。两个官网更新同时争抢同一 Company version 的真实 MySQL 竞争严格收敛为一个完整提交和一个版本冲突；公开 Server E2E 跨重启验证 update/replay/get/rollback/replay/rediscovery 及既有 Source 不变。定向 MySQL、全量 Store race 227.483 秒、Actor race 3.528 秒、vet、diff check 和核心冻结检查均通过，证据见 `evidence/recruiting-company-website-review-20260912.json`；S13 当前验收项关闭。

执行状态补充（2026-09-12，S19 Source 归档恢复与重新校准）：提交 `5e5083ed` 把归档 Source 的规范入口冲突提升为公开、可机读的 `source_restore_required`；detail 指向保留 Source ID/version 和 `source.restore`，拒绝事务不产生重复 Source、receipt 或事件。审计真实旅程发现直接 archive→restore 后没有 pause operation，既有 resume 安全门因此正确拒绝；实现没有绕过安全门，而是让 restore 原子创建使用归档 execution fence 的 cancel-mode scope projection，有界收束旧 Work，完成后才允许 resume。resume 后 Source 仍为 repairing、不会进入 DailyRun；随后公开 validation、真实 Attempt/page/trace Artifact、publish 形成新 assessment，Source 才恢复 active+ready。进程级 E2E 同时证明 Source identity、Assignment 和 Checkpoint lineage 保留、restore 重放在后续状态变化后仍返回原响应。全量 Store race 231.477 秒、Actor race 3.740 秒、vet、diff check 和核心冻结检查通过，证据见 `evidence/recruiting-source-restore-recalibration-20260912.json`；S19 当前验收项关闭。
...
人工处置现通过 `backfill.item.resolve` 支持 `retry|accept_gap`：retry 终结旧 waiting-human Work并创建有因果链接的新 Work，永不重开旧生命周期；预队列版本围栏失败禁止复用已经过时的预览，只能接受缺口或重新建批次。`backfill.pause/resume` 使用 drain 语义，暂停时不产生新物化/offer，但已运行子 Attempt 可按原确认围栏结算。`backfill.cancel` 先提交 `canceling` 栅栏，再由既有 durable reconcile timer 每轮最多 500 项、每项小事务取消 pending/queued/failed Item，拒绝活动 Attempt、释放 Permit，全部收口后才取消父 Work；两项、每页一项的合同证明首批不会提前宣称完成，运行中取消和迟到结果 rejected Artifact 也有覆盖。`backfill.outputs/output.get/gaps` 分别提供元数据分页、最多 1 MiB 的单条正文和失败/accepted-gap 审计报告，避免列表响应装载无界结果。

执行状态补充（2026-09-11，S21 真实纵向退出门）：普通登录用户已通过真实 Atoll Server、daemon 和同一个 `recruiting-executor` class，先完成 MongoDB 公开 Greenhouse 详情页的失败、人工发布修正版 Recipe、因果重试与成功修复，再从该成功 Attempt 的真实 response Artifact 创建并确认一项 `artifact_recompute`，以及从真实 Listing Observation 创建并确认一项 `live_refetch`。公开 `recruiting.system.reconcile` 同时接通预览、取消、物化和 dispatch 的人工运维路径；离线重算 capability 不要求或构造 HTTP Driver/terms 凭证，但仍受同样的 Artifact policy、不可变 offer、预算和结果栅栏约束。两种模式均生成一条独立 BackfillOutput，正文和缺口查询可读；回填前后的 Job state、DetailVersion 数和 Checkpoint version 完全一致，历史重算只声明原 Artifact 观测时刻，实时重抓只声明当前时刻。2026-09-11 实测 49.19 秒通过，证据见 `evidence/recruiting-live-historical-backfill-20260911.json`。结合相关包测试、非 root MySQL 合同、迁移双轮和核心冻结检查，S21 自动化退出门已关闭；第三方站点持续可用性仍属于日常监控，而非一次验收可以证明的事实。
...
进程级控制/数据面容量进展（2026-09-12）：新增版本化 `recruiting.process-capacity-workload.v1`、`make recruiting-process-capacity` 和真实进程用例 `TestRecruitingProcessCapacityThroughRealDataPlanes`。P1 在同一普通用户 Home Channel 中用真实 Server、一个 daemon、四个同 class `company.import` Executor、50 个不可变 File Resource 和非 root MySQL 完成 50 个 preview→人工 confirm→有界 apply 批次，共物化 1,000 Company、1,000 item、1,100 Work 和 100 个成功 Attempt；四个 Executor 均实际领取。协议产生精确 100 条工作唤醒与 100 条 capacity-release 唤醒，200 条 dispatch 全部 delivered；MySQL 中每个已交付领域 event ID 均在 Atoll Channel ledger 中找到。File Resource 共 113,200 字节，写入 427 ms；全部 preview 11.238 秒、全部业务完成 39.101 秒、outbox 排空 39.618 秒，对应完成阶段约 25.57 Company/s。测量点 ledger 有 3,775 条消息、2,169,705 payload bytes、1,241 条 event。Server 重启后 3.214 秒内完成普通用户登录、全部 Executor presence、原命令 byte-stable receipt 重放和 Resource 再读。该测试不访问第三方网站，也明确不以输入 File Resource 冒充响应 Artifact 写吞吐；因此它关闭首个真实 Server/daemon/Executor/ledger/Resource 基线切片，但 S25 仍需 HTTP/Browser 外部执行、响应 Artifact、20K Listing + 400K Detail 完成吞吐及长期 SLO。证据见 `evidence/recruiting-process-capacity-p1-20260912.json`。

Artifact 数据面容量进展（2026-09-12）：新增版本化 `recruiting.artifact-capacity-workload.v1`、`make recruiting-artifact-capacity` 和进程级用例 `TestRecruitingArtifactRecomputeCapacityThroughRealDataPlanes`。A1 由普通用户确认一个冻结 200 个历史 DetailVersion 的 `artifact_recompute` Backfill，四个同 class `recruiting-executor` 全部实际领取；每次执行经 daemon Resource 读取 4 KiB 原始 response、解析冻结 Recipe、写一份 derived Resource，并经公开控制协议提交 BackfillOutput。819,200 输入字节与 23,600 输出字节逐对象回读并和 MySQL SHA-256 metadata 一致，200 Item、200 Attempt、200 Output、200 response Artifact、200 derived Artifact 全部精确成功，活动 Permit 归零。物化使用紧凑唤醒，不为 200 个 Work 各写一条初始消息：精确 4 条 `work_materialized` 和 200 条 `capacity_released` dispatch 全部 delivered；所有已交付领域 event ID 在 Channel ledger 中各存在一次。输入 Resource 写入 1.655 秒，preview 241 ms，业务完成 37.995 秒（约 5.26 derived Artifact/s），outbox 排空 38.462 秒；测量点 ledger 3,438 条消息、2,637,666 payload bytes、324 条 event。Server 重启后 3.508 秒内恢复登录、四 Executor presence、完成态查询、原始创建命令 byte-stable 重放和 Resource 读取。该证据关闭本地历史 response Resource 读取与真实 derived Artifact 写入的容量基线，但没有访问第三方或本地 HTTP/Browser origin，不能冒充 response capture、远程对象存储、20K Listing + 400K Detail 完成或长期 SLO；S25/P9 继续为部分完成。证据见 `evidence/recruiting-artifact-capacity-a1-20260912.json`。

HTTP response capture 容量进展（2026-09-12）：新增 `recruiting.http-capacity-workload.v1`、`make recruiting-http-capacity`、受控 origin fixture 和进程级 `TestRecruitingHTTPResponseCapacityThroughRealDataPlanes`。脚本把 origin 放进 Docker `--internal` 无出站路由网络，并给它只在该网络内可达的公网形状地址；生产 HTTP Driver 未增加 private-network 测试开关，仍执行地址安全、robots、terms、GET-only、Recipe、预算和 circuit 规则。H1 的普通用户确认 200 项 `live_refetch` Backfill，四个同 class `http.fetch` Executor 各只读取一次 robots 并完成精确 200 个 Job GET；origin 记录 200/200 请求。每个 4 KiB response 都先写 daemon File Resource 后才提交结果，200 response Artifact、Attempt、Item 与 BackfillOutput 全部成功，819,200 字节逐对象回读且 SHA-256 metadata 一致，Permit 归零；4 条紧凑初始唤醒与 200 条 capacity-release dispatch 全部 delivered，领域 event 全部在 ledger 各存在一次。容量等待器只读 MySQL 观察 durable timer 协调，并在确认后消费但不依赖实时 feed，避免人工 reconcile 洪泛和 2,048 条测试缓冲反压污染测量。复测 preview 251 ms、业务完成 33.408 秒（约 5.99 response Artifact/s）、outbox 排空 33.814 秒；测量点 ledger 2,991 条消息、2,114,360 payload bytes、303 条 event，Server 重启恢复 2.360 秒。migration 54 在不改变幂等业务时间的前提下增加 MySQL 生成的 offer/accept/start/terminal 观察时间，公开运维投影以最多 1,000 个近期 Attempt 有界统计阶段分位数。H1 的 offer→accept、accept→start、start→terminal、terminal→下一 offer P50 分别为 147/90/127/246 ms；受控 origin 本身只有 5 ms，主要周期成本来自逐岗位控制消息与任务补给，而不是 HTTP。它关闭受控 HTTP JSON response capture 的本机数据面基线，不代表第三方网络、真实延迟/429/CDN 分布、Browser、远程 Artifact provider、H2/5 倍峰值或 L3/L4 全部执行完成；S25/P9 仍为部分完成。证据见 `evidence/recruiting-http-capacity-h1-20260912.json`。

HTTP H2 容量结果与阻塞信号（2026-09-12）：最终 H2 用同一入口把负载扩大到 1,000 Job、8 个 Executor 和每响应 16 KiB。两页各 500 项的有界物化精确产生 16 条 `work_materialized` 唤醒而不是逐 Work 消息，随后 1,000 条 capacity-release 唤醒；1,000 个 origin GET、Attempt、成功 Item、BackfillOutput 和 response Artifact 全部精确一致，16,384,000 字节逐对象回读校验通过，8 个 Executor 均被使用、Permit 归零、领域 event 全部入 ledger、重启恢复通过。在与 H1 相同的无干扰观察方式下，复测 preview 1.004 秒、业务完成 170.171 秒、outbox 排空 171.385 秒，实测约 5.88 response Artifact/s；ledger 为 14,233 条消息和 10,561,848 payload bytes。正确性与 1,000 对象数据面门通过，但从 H1 的 4 Executor/5.99/s 增至 H2 的 8 Executor/5.88/s，吞吐没有提升。H2 的 offer→accept、accept→start、start→terminal、terminal→下一 offer P50 分别膨胀到 319/193/289/482 ms，四段合计约 1.283 秒，和 8 Executor 的实测平台一致；这确认瓶颈是单 Recruiting Actor 上逐岗位串行控制往返与补给，不是人工 reconcile、测试 WebSocket feed 反压或受控 HTTP。若不考虑 Listing 和失败成本而直接按该速率推算，400,000 Detail 约需 18.90 小时；8 小时窗口至少需要 13.89/s。该外推只是定位容量差距，不是生产预测；下一步应在 Recruiting 扩展内实现保持逐 Attempt 状态机与 fence 的紧凑 batch envelope，并复测消息数和吞吐，仍不得修改 Atoll core 或增加 Worker 类型。证据见 `evidence/recruiting-http-capacity-h2-20260912.json`。

HTTP H2 紧凑批量复测（2026-09-12）：Recruiting 扩展新增默认关闭、上限 32 的 execution supply batch；仅允许单结果 `detail_sync`/`historical_backfill_item`，同一个 Executor class 一次领取有界 offer、用逐 Attempt receipt 的复合 `claim` 原子完成 accept+start、逐项执行并批量提交结果。capacity-release dispatch 以 supply batch 为稳定因果键合并，Actor 可沿返回的 continuation dispatch 在一次唤醒内最多继续四批；没有拆分 Actor、增加 Worker 类型或修改 Atoll core。每个 Attempt、Work fence、Permit、结果、Artifact 和失败处理仍独立，默认 batch size 1 保留原协议。相同 H2 输入下，1,000 个 origin GET、Attempt、BackfillOutput 和 16 MiB response Artifact 全部精确成功，8 个 Executor 全部使用，capacity-release 从 1,000 条降至 32 条；业务完成从 170.171 秒降至 74.583 秒，吞吐从 5.88/s 提升到 13.41/s（+128.1%），ledger message 从 14,233 降至 1,638（-88.5%），重启恢复继续通过。batch 阶段观察值包含整批等待和批内顺序执行，不能再解释为逐项网络往返。13.41/s 仍比 400K/8h 的 13.89/s 算术参考低 3.5%，且尚未覆盖 Browser、第三方网络/限流、远程 Artifact、5 倍执行峰值、完整 20K Listing + 400K Detail 与长时间运行，因此 S25 继续为部分完成。证据见 `evidence/recruiting-http-capacity-h2-batch-20260912.json`。

HTTP H3 受控 5 倍执行峰值（2026-09-12）：同一生产 HTTP Driver、32 项 batch、8 Executor 和无出站受控 origin 将 H2 的 1,000 项扩大为 5,000 项；每个响应为 4 KiB，避免把响应体增大与任务峰值混为一个变量。5,000 个 origin GET、Attempt、Item、BackfillOutput 和 response Artifact 全部成功，20,480,000 字节逐对象 hash 校验，8 个 Executor 均被使用、Permit 归零。10 个 500 项物化页精确产生 80 条初始唤醒，157 个 supply batch 产生 157 条 capacity-release，237 条 dispatch 全部 delivered；ledger 中 5,265 条 Recruiting event 完整到达，Server 重启、命令重放和 Resource 再读通过。业务完成 490.119 秒（10.20/s）、outbox 排空 495.894 秒，总测试 561.487 秒。公开执行健康按设计只扫描近期 1,000 个 Attempt，并明确返回 `attempts_truncated=true`。H3 相对 batched H2 吞吐下降 23.9%，按该受控速率完成 400K Detail 约需 10.89 小时，说明单机持续峰值下排队/协调延迟仍是瓶颈。该结果关闭“5 倍受控执行峰值正确性”，不关闭第三方、Browser 同峰值、远程 Artifact、完整 20K Listing + 400K Detail、长时间 SLO 或生产准入；S25/P9 保持部分完成。证据见 `evidence/recruiting-http-capacity-h3-20260912.json`。

Supply batch 预检收敛（2026-09-12）：claim/result handler 不再为 32 个成员分别执行一次批次归属查询，而是在任何状态变化前先完成整批 payload、Attempt 唯一性、Executor incarnation 一致性校验，再用一个最多 32 个主键的查询核对 `supply_batch_id + executor + incarnation`。单项兼容入口保留，逐 Attempt 的状态事务、receipt、fence、Permit 和 Artifact 原子性均未改变；因此第二个成员伪造或缺失时不会在预检阶段之后才发现。非 root MySQL 合同覆盖完整、缺失、重复成员；全量 Store/Actor race 分别为 232.562/3.026 秒，真实 H0 与 H2 数据面通过。H2 为 73.953 秒（13.52/s），相对既有 74.583 秒只快 0.85%，不作为吞吐提升结论。另将每 wake 连续批次数从 4 提到 8 的 H3 对照为 490.034 秒，对比 490.119 秒仅差 0.02%，已完整撤回该无效配置。把 1—32 个 claim 合成一个事务的第二个对照为 74.150 秒，只改善 0.58%，却延长 Work 锁持有期，也已撤回。证据说明剩余瓶颈不是 wake wave 或事务 begin/commit 数，而是批次内部逐 Attempt 的候选发现、fence 和状态查询；下一优化必须针对查询计划，并重新进行 H2/H3 阶梯验证。证据见 `evidence/recruiting-batch-preflight-20260912.json`。

Supply batch 专用候选查询（2026-09-12）：容量协议已冻结批量路径只能领取 `detail_sync|historical_backfill_item`，Repository 因而不再对该路径执行包含 Listing、Discovery、Baseline、Profile、导入和三类 Recipe validation 的通用大 OR 查询。专用 SQL 仍检查 capability、状态、时间、origin/Profile、Company/Source 控制状态、finish-causal-chain、Detail/Backfill owner 和活动 Attempt；选中后继续按主键加锁，并复用原有 scope、完整领域 fence、Profile 设备、预算、不可变 offer 和 Attempt 持久化，普通单项查询不变。非 root MySQL 合同新增 batch-only Detail retry，受控 H0 通过；两个独立 H2 均为 72.620 秒（13.77/s），相对 74.583 秒改善 2.63%，距 400K/8h 算术速率仍差 0.85%。H3 为 5,000/5,000 成功、474.554 秒（10.54/s），相对 490.119 秒改善 3.18%，offer→accept P50/P95 从 7906/14441 降到 7676/13801 ms；按持续 H3 速率 400K 仍约 10.55 小时。完整非 root MySQL Store/Actor race 回归分别为 239.918/3.518 秒。该切片证明缩小候选计划有小幅、可重复收益，但未关闭逐 Attempt fence/状态/result 持久化、外部拓扑和生产吞吐门。证据见 `evidence/recruiting-batch-candidate-query-20260912.json`。

Backfill 成功聚合收敛（2026-09-12）：`live_refetch`/`artifact_recompute` 成功结果不再在父级行锁内为每个 Item 扫描全部 `recruiting_backfill_items`；事务利用已锁定父状态和“一个 receipt 只把一个 queued Item 推进一次”的不变量，单调增加成功计数，把一个 n 项 Backfill 的聚合访问量从 O(n²) 降到 O(n)。失败、accepted gap、取消和修复路径的事实重算保持不变。非 root MySQL 合同覆盖两个不同 Item、逐项重放、父级完成及父聚合与明细四类状态计数严格相等；全量 Store/Actor race 分别为 232.359/3.214 秒，H0、H2、H3 真实进程通过。H2 为 72.079 秒（13.87/s），H3 为 469.316 秒（10.65/s），相对上一版只改善 0.75%/1.10%；这项改动以渐进复杂度安全为接受理由，不包装为吞吐突破。H3 下 400K 推算仍约 10.43 小时，距 8 小时参考差 23.29%，且不能以 Backfill workload 代替每日 Detail 路径测量。证据见 `evidence/recruiting-backfill-aggregate-20260912.json`。

每日 Listing→Detail 容量进展（2026-09-12）：新增版本化 `recruiting.daily-detail-capacity-workload.v1`、`make recruiting-daily-detail-capacity` 和真实进程 `TestRecruitingScheduledDailyDetailCapacityThroughRealDataPlanes`，不再用 Backfill workload 冒充每日结果路径。Atoll durable timer 创建 cutoff 名单和一份 SourceOccurrence；生产 HTTP Listing Driver 在 Docker `--internal` 受控 origin 上按 500 项分页并越过旧活动边界，接受事务写 Observation/Job/Checkpoint 并为 Detail capability 追加至多按可用 Executor 数的紧凑 wake。同一个启用 32 项 batch 的 Executor 遇到非批量安全 Listing 时，控制面不再因空 batch 提前完成 dispatch，Executor 回退到现有单项协议；后续 `detail_sync` 仍走批量协议，没有新增 Actor/Worker 类型或修改 Atoll core。D0/D1/D2/D3 分别完成 20/200/1,000/5,000 个岗位；D3 精确完成 10 次 Listing 请求、5,000 次 Detail GET、5,001 个成功 Attempt、5,000 个 JobDetailVersion 和 5,000 个 4 KiB response Artifact，20,480,000 字节逐 Resource/SHA-256 校验，Permit 归零、所有执行 dispatch delivered、到期领域事件进入 Channel ledger，日报逻辑结算和 Server 重启恢复通过。首个基线 D2 为 77.079 秒（12.97/s），D3 为 423.655 秒（11.80/s）。Recruiting 扩展随后把同一批最多 32 项的 Detail Offer、Claim、Result 收敛为有界数据库事务，并按共享维度聚合预算计数；每项 Attempt、Work、Permit、receipt、Artifact 与结果仍独立持久化，异常快速路径整体回滚后沿用逐项兼容语义。复测 D2 为 60.645 秒（16.49/s），D3 为 351.175 秒（14.24/s），相对 D3 基线提高 17.1%，按该本机受控速率推算 400K 约 7.80 小时。该证据越过 13.89/s 算术参考，但只关闭每日真实数据面的 5,000 项本机受控切片；20K 同时 Listing + 400K Detail 实际执行、第三方延迟/429/CDN、远程 Artifact、外部分片和长期 SLO 尚未完成，S25/P9 继续为部分完成。证据见 `evidence/recruiting-daily-detail-capacity-20260912.json` 与 `evidence/recruiting-daily-detail-transaction-batching-20260912.json`。

Browser response capture 容量进展（2026-09-12）：新增 `recruiting.browser-capacity-workload.v1`、`make recruiting-browser-capacity` 和 HTTP/Browser 共用的真实进程容量旅程。受控 origin 增加有界 HTML 岗位详情；生产 `browser.public` Driver 使用 `/usr/bin/google-chrome`，仍经过公开地址、robots、terms、同源、GET/HEAD-only、无表单/下载/弹窗/写请求约束。首次 B0 暴露出真实业务缺口：Browser 成功会同时产生 DOM response 与 effect trace，而 Detail/Backfill 成功合同曾错误限定只有一个 Artifact，导致 `contract_violated.backfill_live_result` 并安全暂停父 Backfill。修复后主 response 保持唯一，最多 9 个 trace supporting Artifact 与它在同一结果事务持久化、参与重放 hash，fenced 结果也保留全部拒绝证据。B0 4/4、B1 20/20、B2 100/100 均通过；B2 使用 4 个 Executor，精确保存并逐 Resource 校验 100 DOM + 100 trace、1,636,900 response 字节，所有 effect attestation 均无写请求、跨源文档、表单、下载或弹窗，重启和命令重放通过。B1 的 2 Executor 为 1.87/s，B2 的 4 Executor 为 1.88/s，start→terminal P50 从 754ms 增至 1,458ms，说明四核主机已由真实 Chrome CPU/进程成本饱和。Browser 必须按 Recipe capability 进入独立池并优先跨主机扩容，普通岗位继续走 HTTP；在证明独立安全前不启用 Browser batch。该静态 DOM 基线不代表第三方 JavaScript、授权登录、反爬、远程 Artifact 或长期 SLO，S25 仍为部分完成。证据见 `evidence/recruiting-browser-capacity-b2-20260912.json`。
...
HTTP 429/403/5xx 真实进程故障矩阵（2026-09-12）：`make recruiting-daily-detail-failure-matrix` 在同一每日 Listing→Detail 旅程的 Docker `--internal` origin 中固定注入首次 503、首次带 `Retry-After: 1` 的 429 和持续 403。真实 Server、daemon、两个批处理 Executor、File Resource、非 root MySQL 与 Channel ledger 完成 1 次 Listing 和 22 次 Detail 请求；503/429 Work 各只增加一次 `automatic_attempts`，保留 `upstream_5xx`/`throttled` 分类及独立 Failure Artifact 后由新 Attempt 成功，403 精确留下一个 `waiting_human` Detail、一个 open RepairIncident 和一条 affected 关系。最终 19 个 JobDetailVersion/response Artifact 成功，3 个失败 Attempt/Failure Artifact 保留，Permit 与执行 dispatch 归零，日报为 20 expected、19 succeeded、1 exception，Server 重启后摘要和 Resource 可恢复。该切片关闭 P9 点名的 HTTP 状态码进程级注入，不代表第三方分布、熔断开启、Browser/Profile 或远程 Artifact 故障。证据见 `evidence/recruiting-daily-detail-http-failure-matrix-20260912.json`。

Browser 故障注入进展（2026-09-12）：受控 Broker 在导航后、DOM 完成前异常退出时，Driver 把错误稳定分类为 `browser_transport`，并在返回失败前保存绑定原 Attempt 的部分 DOM Artifact；不会把残缺页面送入离线 Recipe 或形成业务结果。Browser Plan 又增加哈希绑定、数量和 CSS 语法受限的 `auth_expired|captcha` selector，真实 Chrome 在动作前后识别并经两层 Driver 保留精确分类。组合合同进一步让 Broker 返回部分 DOM 后崩溃，同时令 Artifact Resource 拒绝 DOM 与 failure evidence；统一 Executor 只完成 `accept/start`，不提交无证据终态，Attempt 留给 TTL/reconcile。新增 BF0 将该语义提升为真实进程：执行 daemon 启动 `/usr/bin/google-chrome`，另一个已认证 daemon 提供 File Artifact；只有在 Attempt running、origin 已接受 Chrome 文档请求且数据库零 Artifact 后才 `SIGKILL` provider。provider 停机 29.844 秒后旧 Attempt expired 且零 accepted Artifact，恢复 dispatch 产生新 Attempt，最终唯一 DOM、trace、BackfillOutput 成功，重启后两类 Resource 均可读。正常 B0 回归通过。该入口关闭“真实 Chrome 正在执行时本地 provider 进程退出”的组合切点，但不冒充同时杀 Chrome/provider、远程对象存储或第三方站点。证据见 `evidence/recruiting-browser-artifact-provider-recovery-20260912.json`。

Artifact provider 故障进展（2026-09-12）：组件合同继续要求 Executor 在证据无法持久化时不得提交无证据的 `execution.failed` 或伪造业务终态；新增 `make recruiting-daily-artifact-recovery` 把该语义提升到独立进程。测试先由单独认证并 attach 的 File provider 写 probe，随后等 Listing Attempt 已 running 且受控 origin 已收到 HTTP 请求，再 `SIGKILL` provider。首次真实运行暴露出仅把旧 dispatch 提前会重放同一 Offer 命令、无法产生新 Attempt：migration 56 因而让 Attempt 持久绑定来源 dispatch，TTL 回收在释放 Permit、把 Work 推进 `waiting_retry` 的同一事务中收口旧 wake，并追加新的 `attempt_recovered` dispatch identity；旧结果继续受 Attempt fence 拒绝。权威复跑中 provider 停机 30.252 秒，旧 Listing Attempt 过期且无 accepted Artifact，新 Attempt 再次抓取 Listing，20 个 Detail、20,480 字节 response Resource、日报结算和 Server 重启后 provider 重连回读全部通过；正常 D0 与 429/403/5xx 矩阵也回归通过。BF0 又证明同一恢复原语适用于真实 Chrome 执行期间的 provider 退出。两项证据关闭本地 File provider 进程断连/恢复及其 Browser 执行组合；远程对象存储和同时杀死多个进程仍未完成。证据见 `evidence/recruiting-daily-artifact-provider-recovery-20260912.json` 与 `evidence/recruiting-browser-artifact-provider-recovery-20260912.json`。

Browser 联合进程恢复进展（2026-09-12）：`make recruiting-browser-joint-process-recovery` 的 BF1 在 Browser Attempt running、origin 已接受真实 Chrome 文档请求且零 Artifact 的同一切点，同时 `SIGKILL` 执行 daemon 进程组和独立 File provider。两个执行侧进程均已停止时，Server 与 Recruiting control 继续运行；设备失联/incarnation fence 在 601 ms 内把旧 Attempt 推进 expired、释放 Permit、收口来源 dispatch 并追加 `attempt_recovered` dispatch，不依赖手工改库。两台 daemon 用原设备身份和持久目录恢复后，Executor 取得新 incarnation，新 Attempt 第二次访问 origin并保存唯一 DOM、effect trace、BackfillOutput；旧 Attempt 零 accepted Artifact，最终 Permit/dispatch 清零，Server 重启后两类 Resource 均可读。BF0 provider-only 与 B0 正常路径均回归通过。该证据关闭本地 Browser 执行进程组与 provider 联合退出切点；Server/MySQL 同时故障、远程 Artifact 和第三方登录站点仍是独立门。证据见 `evidence/recruiting-browser-joint-process-recovery-20260912.json`。
...
正向增量资格进展（2026-09-11）：Women’s Aid 的公开 WordPress Job Listings Endpoint 提供按 `modified_gmt desc` 排序和 offset 分页；当前真实样本包含历史岗位更新后重新置顶的可验证顺序。opt-in 纵向测试从官网发现入口开始，经 Atoll Resource 保存 Recipe 和校验证据，发布 Source 后完成 17 个岗位及其全部详情的 baseline，再运行一次有安全重叠的无变化增量并把 Checkpoint 从 1 推进到 2；baseline 实际读取 4 页，增量只读取顶部至旧活动边界及其后 1 页重叠，共 2 页，且重复命令没有重复事实。该源因此成为当前首个 `production_incremental_eligible=true` 的真实纵向验收样本，而不是把 Greenhouse/Lever 的诊断快照改写成成功。机读证据见 `evidence/recruiting-live-qualified-wordpress-20260911.json`。
...
本阶段只关闭“已发现公开 API 后可保存为低成本 HTTP Listing Recipe 并执行”的数据面能力。网络观察自动沉淀为候选 API Endpoint/Recipe Resource，以及字节 Source 的首次 baseline/Detail 全链路仍是下一切片，不能因 Live Smoke 通过而宣称公司接入完成。2026-09-17 纠偏后，API Endpoint 固化在 Recipe request 中，不再通过 `source.update` 替换人类可访问的 ListURL。
...
新增自然语言 Agent 可调用的 `recruiting.recipe.prepare`。它只接受 active/healthy candidate Source，要求没有 Listing Assignment，以 Source version fence 读取同公司已完成 Probe，并用 body hash 选择一条持久证据。候选必须是 `http.fetch` 的 HTTP JSON Listing Recipe，固定 request URL/method/header/body 与该证据完全一致；Extraction、详情 URL 模板、分页和活动边界继续由共享 ABI 校验。通过后 Recruiting Actor 用规范 Spec content hash 创建不可覆盖、可幂等重放的 content-addressed `recipe://recruiting-prepared/...` KV Resource，生成稳定 Recipe identity，并明确返回 `recipe.propose/validate` 的下一动作。Source endpoint 保持真实 ListURL；同源内部 API 是 Recipe transport，不触发 `source.update`。
...
自动合同覆盖 `Probe evidence → Verification Work offer/accept/start → Artifact result → frozen next Probe offer`，Executor 单元合同覆盖 Artifact-before-result，Browser Executor 合同覆盖 Resource 回读与哈希复核，HTTP Driver 合同覆盖精确 POST 和非 JSON 失败关闭。生产 HTTP Driver 对字节前置配置接口、真实 Browser Broker 对本地回放后发现下游岗位查询的 Live Smoke 均通过。紧凑记录见 `evidence/recruiting-public-query-verification-control-plane-20260914.json`。该阶段关闭可信 Stub 的控制面链，不代表已自动生成 Extraction/pagination、审批 Listing Recipe、发布 Source 或完成字节 baseline；这些仍须走纠偏后的 `recipe.prepare → recipe.propose → recipe.validate → recipe.approve → baseline` 产品状态机，内部 API 不再改变 Source ListURL。

## 26. 2026-09-14 公司名驱动的发现自动推进阶段

本阶段把上两阶段的底层控制词接回自然语言主流程，新增 `recruiting.onboarding.advance`，其公开输入只有 `company_name`。它不是新的调度器：Recruiting Actor 读取当前 Company、最新 Mission，以及至多 Mission 操作上限内的 Probe/Verification/Work 投影，纯函数选择一个下一动作，再复用既有 Mission 预算、Work、dispatch、Attempt、Permit 和结果事务。无 Probe 且 Company 已有确认官网时创建初始 Browser Probe；最新完成 Probe 有尚未验证的公开查询时创建 Verification；响应 Artifact 完成后创建携带累计验证引用的下一 Browser Probe。异步 Work 尚在 open/running/waiting_retry 时只返回等待；paused、waiting_human 或 terminal-but-incomplete 会转人工处理，不制造重复 Work。
