...
进展补充（2026-09-09，替代上一段关于 diagnostic 尚未接通的陈述）：`run.diagnostic` 已接通独立 ListingRun、统一 Work/Attempt/Permit/dispatch、HTTP Executor 和 evidence-only 结果事务；它不进入 DailyRun，也不写 Job、Observation、Detail Work 或 Checkpoint。真实 server/daemon、Greenhouse 公共 API、Recipe KV、Artifact File 与非 root MySQL 旅程已验证：diagnostic 如实进入 `waiting_human/quality_rejected`，且三类业务事实保持零变化。Artifact 采用每 Attempt 独立同层目录，以适配不支持重复/递归建目录的真实设备驱动，同时不修改 Atoll Resource 语义。独立 `run.production` 仍待实现。
