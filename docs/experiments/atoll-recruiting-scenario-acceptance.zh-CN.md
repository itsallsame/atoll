...
| S15 Company 暂停/恢复 | 部分完成 | Company 状态机、原子 `ScopeControlOperation`、500-Work seek 暂停/恢复、逐 Source 短事务 catch-up、冻结 Source 上界、公开 operation/catch-up 查询、跨执行类型的独立配置/取消 fence；真实 cancel operation 联合关闭主要业务执行对象、Attempt、Permit、occurrence，并以独立有界阶段结算未物化 BackfillItem；`TestCompanyResumeProjectsAllAndOnlyItsSourceWorks`、`TestCancelScopeControlClosesExecutionOwnerAggregates`、`TestCancelScopeControlWaitsForBoundedUnmaterializedBackfillItems`、`recruiting-scope-execution-fences-20260912.json` | Recipe/Source onboarding 等上层聚合的取消后恢复语义与完整截点竞态矩阵 |
| S16 Source 暂停/恢复 | 部分完成 | Source 状态机、活动因果根捕获/结算、暂停范围候选隔离、一次当前 production catch-up、Checkpoint 不前移；Listing、Detail、Source Discovery/Validation、三类 Recipe Validation、Baseline、live Backfill 均具备 drain/cancel/config 联合矩阵；Backfill 子 Work 从冻结 Item 强制派生 Company/Source scope，Work 投影完成不再提前宣称 Backfill 取消完成；`TestLiveBackfillResultUsesSeparatedSourceFences`、`TestCancelScopeControlClosesExecutionOwnerAggregates`、`TestCancelScopeControlWaitsForBoundedUnmaterializedBackfillItems`、`recruiting-scope-execution-fences-20260912.json` | Source 隔离下跨 Source 批次的明确处理策略、取消/结果/协调器全截点竞态矩阵 |
