...
| S16 Source 暂停/恢复 | 部分完成 | Source 状态机、活动因果根捕获/结算、暂停范围候选隔离、一次当前 production catch-up、Checkpoint 不前移；Listing、Detail、Source Discovery/Validation、三类 Recipe Validation、Baseline、live Backfill 均具备 drain/cancel/config 联合矩阵；`TestLiveBackfillResultUsesSeparatedSourceFences`、`recruiting-scope-execution-fences-20260912.json` | 通过真实有界 `ScopeControlOperation(cancel)` 驱动全部关联业务聚合、Attempt、Permit 和 occurrence 的统一联合终态及竞态矩阵 |
