...
- migration 33 把 apply request 的稳定业务时间在外部 Assignment 事务之前持久化，关闭“Assignment 已提交、成员尚未登记时进程退出”造成重放 hash 漂移的缺口。Listing Source 对已经选择的同一 Recipe 做 endpoint/实现重校准时复用当前 Assignment，只更新 Source assessment，不制造第二条同 Recipe Assignment 历史；发布事务仍反查成功 Attempt、质量结果和全部 Artifact。
